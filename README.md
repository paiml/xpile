...
- Latest tag: **`v0.1.129`** — **`str.removeprefix`/`removesuffix`** (Python 3.9+). Recent: v0.1.128 tuple literals as list elements (`[(1,2),(3,4)]`), v0.1.127 no-arg `str.split()`, v0.1.126 2-arg `min`/`max` over `str`/`bool`, v0.1.125 `ord(c)` / `chr(n)` builtins, v0.1.124 string iteration (`for c in s`). (v0.1.13 PTX gate.)
