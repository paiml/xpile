...
- Latest tag: **`v0.1.234`** — **`x in range(...)` membership** as a bounds check (`x in range(n)`, `x not in range(a, b, s)`). Recent: v0.1.233 in-place append on a subscript receiver, v0.1.232 in-place set/dict mutators, v0.1.231 tuple-target genexpr/comprehension, v0.1.230 `str` reverse-slice `s[::-1]`, v0.1.229 bare `dict.pop()` statement. (v0.1.13 PTX gate.)
