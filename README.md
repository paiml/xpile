...
- Latest tag: **`v0.1.94`** — Python **`str.format`** (sequential `{}` → Rust `format!`). Recent: v0.1.93 list concatenation `xs + ys`, v0.1.92 `int(s)` / `float(s)` string parsing, v0.1.91 `bool(x)` truthiness cast, v0.1.90 multi-parameter + nullary closures. (v0.1.13 PTX gate.)
