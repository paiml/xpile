...
- Latest tag: **`v0.1.61`** — Python **`map(lambda p: e, xs)`** → `.map(…).collect()`. Recent: v0.1.60 `filter(lambda, xs)`, v0.1.59 `min/max(key=lambda)`, v0.1.58 `sorted(key=lambda)` (first lambda support), v0.1.57 `for k,v in d.items()`. (v0.1.13 PTX gate.)
