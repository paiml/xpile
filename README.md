...
- Latest tag: **`v0.1.45`** — Python **numeric conversions** `int(x)` / `float(x)` (`Expr::NumCast`). Recent: v0.1.44 str methods `.lstrip/.rstrip/.find/.count`, v0.1.43 sequence repetition `"x"*n`, v0.1.42 `all/any` over `list[bool]`, v0.1.41 empty constructors. (v0.1.13 PTX gate.)
