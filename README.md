...
- Latest tag: **`v0.1.95`** — Python **`s.index(sub)`** (substring index, ValueError if absent). Recent: v0.1.94 `str.format` sequential `{}`, v0.1.93 list concatenation `xs + ys`, v0.1.92 `int(s)` / `float(s)` string parsing, v0.1.91 `bool(x)` truthiness cast. (v0.1.13 PTX gate.)
