...
- Latest tag: **`v0.1.204`** — **dataclass methods** (`def m(self,…)` → `impl Name { pub fn m(&self,…) }`, `obj.m(args)` dispatch; dataclasses are now full structs-with-methods). Recent: v0.1.203 dataclass field assignment, v0.1.202 dataclass construction + field access (`Type::Struct`), v0.1.201 `@dataclass` → struct definition, v0.1.200 statement-position assignment-form `try`/`except`, v0.1.199 `try`/`except` via `catch_unwind`. (v0.1.13 PTX gate.)
