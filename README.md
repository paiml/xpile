...
- Latest tag: **`v0.1.25`** — Python **augmented subscript assignment** `d[k] += v` / `xs[i] += v` (desugars to `d[k] = d[k] <op> v`; reuses dict/list machinery, no new IR). Recent: v0.1.24 structured `via_roles`, v0.1.23 enumerate/zip, v0.1.22 DiffExecEngine hook, v0.1.21 slicing. (v0.1.13 PTX gate.)
