...
- Latest tag: **`v0.1.523`** — a `@staticmethod`/`@classmethod` named after a Rust keyword now compiles: its def was `r#`-escaped but the qualified call `Class::match(...)` left the keyword unescaped; the escape pass now escapes the method segment of a `Class::method` callee too (`Reg::r#match`). Recent: v0.1.522 `str.format()` float align/width keeps the trailing `.0`; v0.1.521 an unannotated param's type comes from its default; v0.1.520 method/staticmethod default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.518 the `x if x is not None else d` idiom narrows `x`; v0.1.517 `except E as e:` binds the message; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to dunders; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V24, ~1600 probes; 256+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
