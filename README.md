...
- Latest tag: **`v0.1.166`** — **`xs += ys` list in-place extend** (was silently emitting `checked_add` on a `Vec`). Recent: v0.1.165 nested augmented subscript assignment (`grid[i][j] += v`) + latent `let mut` mutability fix, v0.1.164 `for _ in range(n)` underscore loop targets, v0.1.163 nested subscript assignment (`grid[i][j] = v`), v0.1.162 mixed `{**a, "k": v}` dict literals, v0.1.161 `{**d1, **d2}` dict merge. (v0.1.13 PTX gate.)
