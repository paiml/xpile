...
- Latest tag: **`v0.1.82`** — Python **dict get-or-insert `d.setdefault(k, default)`**. Recent: v0.1.81 str padding `s.rjust(w)` / `s.ljust(w)`, v0.1.80 set removal `s.remove(x)` / `s.discard(x)`, v0.1.79 dict pop `d.pop(k)`, v0.1.78 item deletion `del coll[key]`. (v0.1.13 PTX gate.)
