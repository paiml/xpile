...
- Latest GitHub tag: **`v0.1.601`** — see [`CHANGELOG.md`](CHANGELOG.md) for the full per-tag history (one entry per slice). The recent cadence pairs a sustained run of correctness fixes from multi-pass adversarial differential `python3`-vs-`rustc` hunts (HUNT-V1…V33, ~2000+ probes) — bringing the transpiled Rust into exact agreement with CPython across an expanding Python subset — with the Sprint turn to the two north-star differentiators: a differential **oracle** and **hybrid** transpilation. The hybrid north star is now *executed*: `xpile hybrid <dir> --verify` emits a buildable Cargo workspace, `cargo build`s it (a `build.rs` cc-compiles + links the emitted `unsafe extern "C"` shim), runs the linked C+shim binary, and **differential-matches it byte-for-byte against the CPython reference** (the same cc-compiled C extension bound via `ctypes`) — a Run≥1 single-fixture witness that the emitted FFI shim is ABI-faithful. The proof lane is now **machine-checked**: an advisory `lake build` CI job elaborates an 11-module Lean pilot under a pinned toolchain (`grep sorry` is no longer the provability oracle). All under the R6 contract-integrity regime so every emitted construct cites a provable contract.
- Published on crates.io: [`xpile`](https://crates.io/crates/xpile) (`cargo install xpile`) — all 27 workspace crates. Per the release cadence, crates.io publishes **once per week on Fridays** while GitHub tags ship per slice, so the crates.io line catches up to the latest tag each Friday. The 2026-06-19 batch publishes **v0.1.536** (previous batch: v0.1.37, 2026-06-12).
...
### Emitted-code dependencies

The generated Rust is std-only **except** for Python `dict`, which lowers to
[`indexmap::IndexMap`](https://docs.rs/indexmap) so iteration follows Python's
insertion order (a plain `HashMap` iterates non-deterministically — see
v0.1.573 / PMAT-874). If your transpiled output uses any dict, add to its
`Cargo.toml`:
...
Output that uses no dict needs no extra dependencies.
