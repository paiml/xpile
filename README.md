...
- Latest tag: **`v0.1.182`** — **`min(xs)`/`max(xs)` over a `list[str]`** (str/bool are Ord). Recent: v0.1.181 collection `.copy()` (`list`/`dict`/`set`), v0.1.180 set predicates (`issubset`/`issuperset`/`isdisjoint` + `<=`/`<`/`>=`/`>`), v0.1.179 set-algebra methods (`union`/`intersection`/`difference`/`symmetric_difference`), v0.1.178 2-arg `math` (`hypot`/`atan2`/`log(x,base)`), v0.1.177 `math.pow` + `math.trunc`. (v0.1.13 PTX gate.)
