...
- Latest tag: **`v0.1.522`** — `str.format()` with an align/width-only spec on a float keeps Python's trailing `.0`: `"{:>6}".format(3.0)` used Rust's bare f64 Display ("     3" vs "   3.0"); the float is now rendered to its repr string first and padded (the str.format analog of the v0.1.507 f-string fix). Recent: v0.1.521 an unannotated param's type comes from its default; v0.1.520 method/staticmethod default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.518 the `x if x is not None else d` idiom narrows `x`; v0.1.517 `except E as e:` binds the message; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to dunders; v0.1.514 `dict(d)` is an independent copy; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V24, ~1600 probes; 255+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
