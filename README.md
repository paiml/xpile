...
- Latest tag: **`v0.1.15`** — sprint slice PMAT-492: Python **no-arg string methods** `.upper()/.lower()/.strip()` → `.to_uppercase()/.to_lowercase()/.trim().to_string()` (new meta-HIR `Expr::StrMethod`). (v0.1.14 WGSL gate; v0.1.13 PTX gate; v0.1.12 early returns; v0.1.11 C if/else; v0.1.10 float.)
