...
- Latest tag: **`v0.1.146`** — **`min`/`max(xs, default=)`** (empty-safe). Recent: v0.1.145 filtered generator expressions, v0.1.144 generator expressions, v0.1.143 context-aware subscript index + unary `-` (closes the ctx-free-position class), v0.1.142 context-aware collection literals, v0.1.141 context-aware comparison operands. (v0.1.13 PTX gate.)
