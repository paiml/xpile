...
- Latest GitHub tag: **`v0.1.600`** — see [`CHANGELOG.md`](CHANGELOG.md) for the full per-tag history (one entry per slice). The recent cadence pairs a sustained run of correctness fixes from multi-pass adversarial differential `python3`-vs-`rustc` hunts (HUNT-V1…V33, ~2000+ probes) — bringing the transpiled Rust into exact agreement with CPython across an expanding Python subset — with the Sprint turn to the two north-star differentiators: a differential **oracle** and **hybrid** transpilation. The hybrid north star is now *executed*: `xpile hybrid <dir> --verify` emits a buildable Cargo workspace, `cargo build`s it (a `build.rs` cc-compiles + links the emitted `unsafe extern "C"` shim), runs the linked C+shim binary, and **differential-matches it byte-for-byte against the CPython reference** (the same cc-compiled C extension bound via `ctypes`) — a Run≥1 single-fixture witness that the emitted FFI shim is ABI-faithful. All under the R6 contract-integrity regime so every emitted construct cites a provable contract.
