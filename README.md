...
- Latest tag: **`v0.1.192`** — **bitwise invert `~x`** (Python `~x` == `-(x+1)` == Rust `!x` on a signed int). Recent: v0.1.191 Optional intra-branch narrowing (`if x is not None:`), v0.1.190 Optional flow-narrowing (cut 4 keystone), v0.1.189 1-arg `d.get(k)` → `Optional[V]`, v0.1.188 Optional params + `is None` tests, v0.1.187 `Optional[T]` return type. (v0.1.13 PTX gate.)
