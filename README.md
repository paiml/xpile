...
- Latest tag: **`v0.1.14`** — second **§30 Track 4** slice (PMAT-482): a pure-Rust **WGSL well-formedness validator** (`validate_wgsl`/`wgsl_looks_real`), mirroring v0.1.13's **PTX validator** (`validate_ptx`/`ptx_looks_real`/`ptxas_arch`). The §29 quorum's offline, free-CI gates for both GPU lanes. (v0.1.12 early returns; v0.1.11 C if/else; v0.1.10 float; v0.1.9 kwargs.)
- Published on crates.io: [`xpile 0.1.13`](https://crates.io/crates/xpile) (`cargo install xpile`). Per the release cadence, crates.io publishes **once per week on Fridays**; GitHub tags ship per slice — so the crates.io line catches up to the latest tag each Friday (next window 2026-06-19).
