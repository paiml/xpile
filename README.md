...
- Latest tag: **`v0.1.165`** — **nested augmented subscript assignment** (`grid[i][j] += v`); also fixes a latent `let mut` mutability gap for literal-initialised subscript-mutated receivers. Recent: v0.1.164 `for _ in range(n)` underscore loop targets, v0.1.163 nested subscript assignment (`grid[i][j] = v`), v0.1.162 mixed `{**a, "k": v}` dict literals, v0.1.161 `{**d1, **d2}` dict merge, v0.1.160 expression-position set/dict comprehensions. (v0.1.13 PTX gate.)
