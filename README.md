...
- Latest tag: **`v0.1.262`** — multiple `if` filters in comprehensions (`[x for x in xs if a if b]`). Recent: v0.1.261 three-way `zip`, v0.1.260 in-place keyed sort `xs.sort(key=lambda)`, v0.1.259 correctness negative-index assignment `xs[-1] = v`, v0.1.258 subscript-target swap `xs[i], xs[j] = xs[j], xs[i]`, v0.1.257 f-string percent `f"{x:.1%}"`. (v0.1.13 PTX gate.)
