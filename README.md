...
- Latest tag: **`v0.1.525`** — `map`/`filter`/`sum` over a homogeneous tuple now works: `",".join(map(str, t))` over a tuple emitted a bogus `map(str, t)` call; a homogeneous tuple is now materialized to a list (a literal reuses its elements, a variable indexes each field). Recent: v0.1.524 two sibling nested `range()` loops reusing the same inner var name compile; v0.1.523 a `@staticmethod` named a Rust keyword is `r#`-escaped at the call site; v0.1.522 `str.format()` float align keeps the `.0`; v0.1.521 an unannotated param's type comes from its default; v0.1.520 method/staticmethod default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.518 `x if x is not None else d` narrows `x`; v0.1.517 `except E as e:` binds the message; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to dunders; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V24, ~1600 probes; 258+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
