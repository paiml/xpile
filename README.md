...
- Latest tag: **`v0.1.96`** — Python **module-level constants** `NAME = <int/bool/float literal>` → Rust `const`. Recent: v0.1.95 `s.index(sub)`, v0.1.94 `str.format` sequential `{}`, v0.1.93 list concatenation `xs + ys`, v0.1.92 `int(s)` / `float(s)` string parsing. (v0.1.13 PTX gate.)
