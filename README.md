...
- Published: [`xpile 0.1.4`](https://crates.io/crates/xpile) — extends the `decy` C → Rust frontend to **iterative** programs: `while` loops, variable reassignment (with correct `let mut` inference), and C truncating `/` `%` (`wrapping_div`/`wrapping_rem`, e.g. `-7/2 == -3` not floor `-4`). Iterative `sum_to` round-trips through rustc (`sum_to(100) == 5050`). (v0.1.3 added the C frontend itself — `int` functions, recursion, ternary, `+ - *`, `i32`+`wrapping_*` C semantics; v0.1.2 completed the Python **dict operations** lane.) All 27 workspace crates at 0.1.4; `cargo install xpile` upgrades cleanly.
