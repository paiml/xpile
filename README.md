...
- Published: [`xpile 0.1.8`](https://crates.io/crates/xpile) — adds Python **list comprehensions** `[elem for var in iter]` (materialised to `tmp = []` + a for-append loop, in return and assignment position). (v0.1.7 added dict iteration; v0.1.6 cross-function inference; v0.1.5 augmented assignment; v0.1.4 iterative C; v0.1.3 the C frontend.) All 27 workspace crates at 0.1.8; `cargo install xpile` upgrades cleanly.
