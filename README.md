...
- Published: [`xpile 0.1.5`](https://crates.io/crates/xpile) — adds Python **augmented assignment** (`x += 1`, `-= *= //= %= &= |= ^= <<= >>= **=`), desugared to `x = x <op> e` with overflow-checking + str-concat detection intact (`s += "!"` → `format!`, `p *= x` → `checked_mul`) and correct `let mut` inference — unblocking the most-used Python loop idiom. (v0.1.4 added iterative C; v0.1.3 the C frontend; v0.1.2 Python dict operations.) All 27 workspace crates at 0.1.5; `cargo install xpile` upgrades cleanly.
