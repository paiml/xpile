...
- Latest tag: **`v0.1.53`** — Python **list query methods** `xs.count(x)` / `xs.index(x)`. Recent: v0.1.52 reverse-slice `xs[::-1]`, v0.1.51 negative list indexing `xs[-1]`, v0.1.50 open-ended slices `xs[a:]`/`xs[:b]`/`xs[:]`, v0.1.49 tuple indexing `t[0]`. (v0.1.13 PTX gate.)
