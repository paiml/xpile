...
- Latest tag: **`v0.1.227`** — **container truthiness** (`if xs:` / `while q:` / `x if xs else y` / `not d`). Recent: v0.1.226 `map`/`filter` typed lambda, v0.1.225 comprehension/genexpr typed elements, v0.1.224 `sorted`/`min`/`max` tuple key, v0.1.223 negative-step `range`, v0.1.222 `len`/`sorted`/`reversed`(range) + `list(dict)`. (v0.1.13 PTX gate.)
