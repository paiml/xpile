...
- Latest tag: **`v0.1.189`** — **1-arg `d.get(k)` → `Optional[V]`** (Optional epic: no-default dict get returns `Option<V>` via `.get(&k).cloned()`, plus a no-double-wrap fix so returning an already-`Optional` value passes through). Recent: v0.1.188 Optional params + `is None` tests, v0.1.187 `Optional[T]` return type, v0.1.186 `sorted(s)` over a str, v0.1.185 `sorted(d)` over a dict, v0.1.184 set splat literals (`{*a, *b}`). (v0.1.13 PTX gate.)
