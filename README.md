...
- Latest tag: **`v0.1.274`** — correctness/contract: left-shift value overflow (`1 << 63`) now panics per C-PY-INT-ARITH instead of silently wrapping to `i64::MIN` (`checked_shl` only guards the shift amount). Recent: v0.1.273 mutating method (`xs.pop()`) in a `while`/`if`/`assert` condition marks its receiver `mut` (was rustc E0596), v0.1.272 Rust-keyword identifiers (`type`/`match`/`loop`/`move` → `r#name`), v0.1.271 tuple reassignment in a loop/if body (`a, b = b, a % b` — Euclid GCD / Fibonacci), v0.1.270 3-arg `pow(a, b, m)` modpow, v0.1.269 negative `xs.pop(-k)`/`del xs[-k]`. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, list-repeat, neg pop/del, modpow, loop-var reassignment, keyword identifiers, mut-receiver-in-condition, left-shift overflow).
