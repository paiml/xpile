...
- Latest tag: **`v0.1.264`** — correctness: `bool` is an `int` subtype (`True+True`, `sum(x>0 for x in xs)`). Recent: v0.1.263 `len(str)` counts Unicode chars, v0.1.262 multiple `if` filters in comprehensions, v0.1.261 three-way `zip`, v0.1.260 in-place keyed sort `xs.sort(key=lambda)`, v0.1.259 negative-index assignment. (v0.1.13 PTX gate.)
