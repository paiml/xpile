...
- Latest tag: **`v0.1.16`** — Python **`startswith`/`endswith`** string predicates → `.starts_with(&(p)[..])`/`.ends_with(...)` (Bool), extending v0.1.15's `Expr::StrMethod` (`.upper()/.lower()/.strip()`). (v0.1.14 WGSL gate; v0.1.13 PTX gate; v0.1.12 early returns; v0.1.11 C if/else.)
