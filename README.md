...
- Published: [`xpile 0.1.13`](https://crates.io/crates/xpile) — opens the **§30 Track 4** GPU Runtime-stratum work (PMAT-481): a pure-Rust **PTX well-formedness validator** (`validate_ptx`/`ptx_looks_real`/`ptxas_arch`) — the §29 quorum's offline, free-CI gate, with `ptxas -arch` derived from `compute_capability`. (v0.1.12 early returns; v0.1.11 C if/else; v0.1.10 float; v0.1.9 kwargs; v0.1.8 comprehensions.) All 27 workspace crates at 0.1.13; `cargo install xpile` upgrades cleanly.
