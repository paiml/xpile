...
- Latest tag: **`v0.1.196`** — **R6: author the `C-C-INT-ARITH` contract** at depth-1 (closes half the §7.3 "every construct under contract" falsification — emitted C now resolves its citation to a real YAML + Lean theorem; first contract to join via the grandfathered gate). Recent: v0.1.195 R6 grandfather the Diamond depth-13 gate, v0.1.194 two-generator dict & set comprehensions, v0.1.193 two-generator list comprehension, v0.1.192 bitwise invert `~x`, v0.1.191 Optional intra-branch narrowing. (v0.1.13 PTX gate.)
