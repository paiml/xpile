...
- Latest tag: **`v0.1.145`** — **filtered generator expressions** (`sum(x for x in xs if x > 0)`). Recent: v0.1.144 generator expressions, v0.1.143 context-aware subscript index + unary `-` (closes the ctx-free-position class), v0.1.142 context-aware collection literals, v0.1.141 context-aware comparison operands, v0.1.140 context-aware ternary branches. (v0.1.13 PTX gate.)
