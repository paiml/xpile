...
- Latest tag: **`v0.1.489`** — a typed `except` is now an allowlist: it was a blocklist (re-raise only the other cataloged builtins, catch everything else), so `except ValueError:` silently swallowed a `RuntimeError` / any non-cataloged or untagged exception where Python propagates; both backends now catch iff the panic tag names one of the handler's own listed types and re-raise the rest (a bare `except:` still catches all), so a `RuntimeError` propagates past `except ValueError` and `except RuntimeError` now catches the dict-size-change error (previously uncatchable). Recent: v0.1.488 a failed `assert` raises a catchable `AssertionError`; v0.1.487 a bool key in a `dict[int, V]` literal is coerced to i64; v0.1.486 a non-None value to an Optional dataclass field is Some-wrapped; v0.1.485 arith/bitwise ops (`//`/`%`/`**`/`<<`/`&`/…) dispatch to user dunders; v0.1.484 a list/str/dict loop variable leaks to the enclosing scope; v0.1.483 a `math.*` builtin coerces an int arg to f64; v0.1.482 int literals in a `list[float]` coerce to f64; v0.1.481 an int arg to a float param widens; v0.1.480 a nested def whose body type ≠ its declared return is rejected; v0.1.479 `str(s)` over a str is the identity; v0.1.478 `repr()` escapes control chars as `\xNN`; v0.1.477 a custom `__ne__` drives `!=`; v0.1.476 a custom `__str__` drives `Display`; v0.1.471 the 7-method dunder-dispatch cluster (`__eq__`/`__len__`/`__getitem__`/`__add__`/`__lt__`/`__call__`/`__contains__`) completed; v0.1.465 `for i in range(len(xs))` mutating `xs` no longer loops forever; v0.1.445 int↔float comparison is exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1..V17, ~900 probes).
