...
- Latest tag: **`v0.1.230`** — **`str` reverse-slice** (`s[::-1]`). Recent: v0.1.229 bare `dict.pop()` statement, v0.1.228 bare `xs.pop()` statement, v0.1.227 container truthiness, v0.1.226 `map`/`filter` typed lambda, v0.1.225 comprehension/genexpr typed elements. (v0.1.13 PTX gate.)
