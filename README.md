...
- Latest tag: **`v0.1.512`** — `set("abc")` / `set((1, 2, 3))` now compile: the `set()`/`frozenset()` constructor only accepted a list, but Python iterates any iterable (a string yields its chars, a tuple its elements); the ctor now materializes a string to its chars and a tuple literal to a list before building the set. Recent: v0.1.511 the `dict(a=1, b=2)` keyword constructor lowers to a string-keyed dict literal; v0.1.510 a dataclass field named after a Rust keyword (`type`/`match`/`ref`) is `r#`-escaped (label-stripped in the repr); v0.1.509 a walrus-bound name reassigned once gets `let mut`; v0.1.508 a dataclass with a custom `__hash__` is usable as a set element / dict key; v0.1.507 a float with an align/width-only spec keeps Python's trailing `.0`; v0.1.506 a bare `range(...)` bound to a variable materializes to a `Vec<i64>`; v0.1.505 comprehension tuple loop vars no longer pollute function scope; v0.1.503 `except LookupError`/`ArithmeticError` catch only their member exceptions; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V22, ~1400 probes; 242+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
