...
- Latest tag: **`v0.1.178`** — **2-arg `math`**: `hypot`/`atan2`/`log(x,base)`. Recent: v0.1.177 `math.pow` + `math.trunc`, v0.1.176 more `math` (constants `pi`/`e`/`tau` + `sin`/`cos`/`tan`/`exp`/`log`/`log10`/`log2`), v0.1.175 `math.sqrt`/`floor`/`ceil` + `import math`, v0.1.174 direct-index of a block-producing collection (`sorted(xs)[0]`), v0.1.173 bare callable as `key=` for `min`/`max`/`sorted`. (v0.1.13 PTX gate.)
