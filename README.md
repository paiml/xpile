...
- Latest tag: **`v0.1.516`** — a for-loop that mutates each element in place (`for row in grid: row.append(x)`, `row[i] = v`) now compiles and propagates: it emitted `grid.iter().cloned()` (mutating a discarded clone, and `row` not `mut` → rustc E0596); it now binds `row` by `&mut` via `grid.iter_mut()`, so the mutation reaches the original collection. Recent: v0.1.515 unary `-obj`/`~obj` over a custom class dispatch to `__neg__`/`__invert__`; v0.1.514 `dict(d)` over a dict is an independent copy; v0.1.513 a method named a Rust keyword is `r#`-escaped at def + call; v0.1.512 `set("abc")`/`set((1,2,3))` materialize str-chars / tuple-elements; v0.1.511 `dict(a=1, b=2)` lowers to a string-keyed dict literal; v0.1.510 a dataclass field named a Rust keyword is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is usable as a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V23, ~1500 probes; 249+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
