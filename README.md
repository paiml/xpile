...
- Latest tag: **`v0.1.221`** — **`sum(range(…))` / `sum`/`max`/`min(set(…))`** (reductions over non-list iterables; fixes a silent miscompile). Recent: v0.1.220 `list(set)`/`sorted(set)`, v0.1.219 `frozenset`, v0.1.218 `str.split` maxsplit, v0.1.217 `str.replace` 3-arg, v0.1.216 `str.startswith`/`endswith` tuple. (v0.1.13 PTX gate.)
