...
- Latest tag: **`v0.1.287`** — correctness: a non-Copy variable passed by value to a call and reused (`helper(xs) + helper(xs)`) now clones the reused arg instead of failing with E0382 (move-after-use) — second ownership-cluster fix. Recent: v0.1.286 a class/enum named after a prelude type xpile emits is rejected cleanly (was E0107), v0.1.285 `int(inf)`/`int(nan)` panic instead of silently saturating, v0.1.284 a non-Copy field returned from `&self` clones instead of E0507, v0.1.283 float `sum()` uses CPython 3.12+ Neumaier compensation, v0.1.282 float scientific notation. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite, prelude-type-name reject).
