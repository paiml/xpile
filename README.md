...
- Latest tag: **`v0.1.174`** — **direct-index of a block-producing collection** (`sorted(xs)[0]`, `reversed(xs)[0]`) — was a silent rustc-failure. Recent: v0.1.173 bare callable as `key=` for `min`/`max`/`sorted`, v0.1.172 `d.setdefault(k, v)` as a bare statement, v0.1.171 `xs.remove(x)` list remove-by-value, v0.1.170 `float` f-string field → Python repr `3.0`, v0.1.169 `bool` f-string field → `True`/`False`. (v0.1.13 PTX gate.)
