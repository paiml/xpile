...
- Latest tag: **`v0.1.106`** — Python **float power** (`x ** 2`, `2.0 ** -1`). Recent: v0.1.105 true division (`/` → float), v0.1.104 float floor-division / modulo (`//`, `%`), v0.1.103 float augmented assignment (`x += y`), v0.1.102 float-variable negation (`-x`), v0.1.101 negative float literals. Float arithmetic family now complete. (v0.1.13 PTX gate.)
