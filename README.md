...
- Latest tag: **`v0.1.28`** — Python **general `if/else`** with side-effecting branches (histogram `if w in freq: freq[w] += 1 …` now lowers to a real `Stmt::If`, not just the value-producing if-as-let). Recent: v0.1.27 `sum`, v0.1.26 abs/min/max, v0.1.25 aug-subscript-assign, v0.1.24 structured `via_roles`. (v0.1.13 PTX gate.)
