...
- Latest tag: **`v0.1.202`** — **dataclass construction + field access** (classes epic: `Point(a, b)` → struct literal, `p.x` → field read, via a new `Type::Struct`; dataclasses now usable end-to-end). Recent: v0.1.201 `@dataclass` → struct definition, v0.1.200 statement-position assignment-form `try`/`except`, v0.1.199 `try`/`except` value-with-fallback via `catch_unwind`, v0.1.198 reject `tuple(<iterable>)` cleanly, v0.1.197 R6 COMPLETE. (v0.1.13 PTX gate.)
