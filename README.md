...
- Latest tag: **`v0.1.299`** — correctness: `str.isspace()` and `strip`/`lstrip`/`rstrip` now treat the C0 information separators U+001C..U+001F as whitespace (Python does; Rust's `char::is_whitespace()`/`trim` excludes them), so `"\x1c".isspace()` and `"\x1cabc".strip()` match python3. Recent: v0.1.298 a dict comprehension reusing a non-Copy loop var in both key and value (`{w: w for w in words}`) clones the key (was E0382), v0.1.297 an empty `set()` infers its element type from the subsequent `.add(...)` (was `HashSet<i64>` default E0308), v0.1.296 the standalone `format(value[, spec])` builtin (was a bare `format(...)` macro-call E0423), v0.1.295 `reversed(s)` over a `str` reverses its characters (was a bare `reversed(...)` E0425), v0.1.294 integer `sum()` / `enumerate(xs, start)` honor the C-PY-INT-ARITH overflow contract (was silent `-O` wrap). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite + out-of-range, prelude-type-name reject, list.insert index clamp, float-mod fmod parity, frozen-dataclass Eq+Hash, PEP 584 dict union, enumerate start= keyword, int sum/enumerate overflow contract, reversed(str), format() builtin, empty-set element inference, dict-comp key clone, C0-separator whitespace).
