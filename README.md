...
- Latest tag: **`v0.1.239`** — **correctness: mixed `float`/`int` compare + arithmetic** (`x == 3`, `x * 2 + 1` over a float now compile). Recent: v0.1.238 negative/OOB slice bounds, v0.1.237 `//`/`%` with a negative divisor, v0.1.236 keyword form of `str.format`, v0.1.235 `int(b)`/`float(b)` over a bool, v0.1.234 `x in range(...)` membership. (v0.1.13 PTX gate.)
