...
- Latest tag: **`v0.1.226`** — **`map`/`filter` lambda over typed elements** (`map(lambda p: p[0]+p[1], ps)`; fixes a silent miscompile). Recent: v0.1.225 comprehension/genexpr typed elements, v0.1.224 `sorted`/`min`/`max` tuple key, v0.1.223 negative-step `range`, v0.1.222 `len`/`sorted`/`reversed`(range) + `list(dict)`, v0.1.221 `sum`/`max`/`min` over range/set. (v0.1.13 PTX gate.)
