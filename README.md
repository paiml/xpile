...
- Published: [`xpile 0.1.11`](https://crates.io/crates/xpile) — adds **C if/else statements** (`Stmt::If`): the decy frontend’s first statement-level branching (`max3`, `clamp` round-trip through rustc), with `mut` inference for locals reassigned in a branch. (v0.1.10 float; v0.1.9 keyword args; v0.1.8 list comprehensions; v0.1.7 dict iteration; v0.1.3/1.4 the C frontend.) All 27 workspace crates at 0.1.11; `cargo install xpile` upgrades cleanly.
