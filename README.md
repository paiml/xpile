...
- Latest tag: **`v0.1.268`** — correctness: list-of-list repeat `[[0]] * n` compiles. Recent: v0.1.267 `max(key=)`/`sorted(reverse=)` tie+stability, v0.1.266 str slicing by Unicode char, v0.1.265 `str.find/index` char index, v0.1.264 `bool` is an `int` subtype, v0.1.263 `len(str)` counts Unicode chars. (v0.1.13 PTX gate.) A run of recent correctness fixes came from an adversarial differential python3-vs-rustc hunt (str Unicode, bool-as-int, tie/stability, list-repeat).
