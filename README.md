...
- Latest tag: **`v0.1.255`** — expr-position **2-generator** genexpr/comprehension (`sum(i*j for i in range(n) for j in range(m))`). Recent: v0.1.254 in-place `xs.sort(reverse=True)`, v0.1.253 `math.perm(n, k)`, v0.1.252 `math.comb(n, k)`, v0.1.251 `math.isqrt(n)`, v0.1.250 `math.factorial(n)`. (v0.1.13 PTX gate.)
