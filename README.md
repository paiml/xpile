...
- Latest tag: **`v0.1.169`** — **`bool` in an f-string renders Python-style `True`/`False`** (was a silent "true"/"false" miscompile). Recent: v0.1.168 f-string lone `{n}` field + integer radix/width specs, v0.1.167 empty list literal `[]` annotation threading, v0.1.166 `xs += ys` list in-place extend, v0.1.165 nested augmented subscript assignment (`grid[i][j] += v`) + latent `let mut` fix, v0.1.164 `for _ in range(n)` underscore loop targets. (v0.1.13 PTX gate.)
