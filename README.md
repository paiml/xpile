...
- Latest GitHub tag: **`v0.1.596`** — see [`CHANGELOG.md`](CHANGELOG.md) for the full per-tag history (one entry per slice). The recent cadence is a sustained run of correctness fixes from multi-pass adversarial differential `python3`-vs-`rustc` hunts (HUNT-V1…V29, ~2000 probes) that bring the transpiled Rust into exact agreement with CPython across an expanding Python subset, now turning to contract-integrity work (R6) so every emitted construct cites a provable contract.
