...
- Latest tag: **`v0.1.279`** — correctness: `bool & | ^ bool` now stays a bool (was inferred int → a `-> bool` function was rejected). Recent: v0.1.278 `abs()` of an i64 overflow-checks (`abs(i64::MIN)` panics per C-PY-INT-ARITH), v0.1.277 `sorted()` over a `list[float]` compiles (was E0277; uses `partial_cmp`), v0.1.276 `x >> n` for `n ≥ 64` saturates to the sign fill like Python, v0.1.275 chained comparison (`0 < xs.pop() < 100`) evaluates each operand exactly once, v0.1.274 left-shift value overflow (`1 << 63`) panics instead of silently wrapping. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type).
