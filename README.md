...
- Latest tag: **`v0.1.180`** — **set predicates**: `issubset`/`issuperset`/`isdisjoint` + `<=`/`<`/`>=`/`>` (the operators were a silent miscompile). Recent: v0.1.179 set-algebra methods (`union`/`intersection`/`difference`/`symmetric_difference`), v0.1.178 2-arg `math` (`hypot`/`atan2`/`log(x,base)`), v0.1.177 `math.pow` + `math.trunc`, v0.1.176 more `math` (constants + trig/log), v0.1.175 `math.sqrt`/`floor`/`ceil` + `import math`. (v0.1.13 PTX gate.)
