...
- Latest tag: **`v0.1.198`** — **reject `tuple(<iterable>)` cleanly** (was a silent miscompile → undefined `tuple(...)` Rust call; now a clear lowering error — upholds "transpile-success ⟹ valid Rust"). Recent: v0.1.197 R6 COMPLETE (`C-XLATE-PY-DICT-TO-HASHMAP`), v0.1.196 `C-C-INT-ARITH` contract, v0.1.195 R6 grandfather the Diamond depth-13 gate, v0.1.194 two-generator dict & set comprehensions, v0.1.193 two-generator list comprehension. (v0.1.13 PTX gate.)
