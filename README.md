...
- Latest tag: **`v0.1.267`** — correctness: `max(key=)` first-on-tie + `sorted(reverse=True)` stability. Recent: v0.1.266 str slicing by Unicode char, v0.1.265 `str.find/index` char index, v0.1.264 `bool` is an `int` subtype, v0.1.263 `len(str)` counts Unicode chars, v0.1.262 multiple `if` filters in comprehensions. (v0.1.13 PTX gate.) A run of recent correctness fixes came from an adversarial differential python3-vs-rustc hunt (str Unicode, bool-as-int, tie/stability semantics).
