...
- Latest tag: **`v0.1.233`** — **in-place append on a subscript receiver** (`g[i].append(e)`, `d[k].append(e)`). Recent: v0.1.232 in-place set/dict mutators, v0.1.231 tuple-target genexpr/comprehension, v0.1.230 `str` reverse-slice `s[::-1]`, v0.1.229 bare `dict.pop()` statement, v0.1.228 bare `xs.pop()` statement. (v0.1.13 PTX gate.)
