...
- Latest tag: **`v0.1.191`** — **Optional intra-branch narrowing** (`if x is not None:` narrows `x` to `T` inside the then-branch — complement of the cut-4 early-return guard). Recent: v0.1.190 Optional flow-narrowing (cut 4 keystone), v0.1.189 1-arg `d.get(k)` → `Optional[V]`, v0.1.188 Optional params + `is None` tests, v0.1.187 `Optional[T]` return type, v0.1.186 `sorted(s)` over a str. (v0.1.13 PTX gate.)
