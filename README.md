...
- Latest tag: **`v0.1.51`** — Python **negative list indexing** `xs[-1]` → `xs[len(xs)-1]`. Recent: v0.1.50 open-ended slices `xs[a:]`/`xs[:b]`/`xs[:]`, v0.1.49 tuple indexing `t[0]`, v0.1.48 chained comparisons `a < b < c`, v0.1.47 substring containment `sub in s`. (v0.1.13 PTX gate.)
