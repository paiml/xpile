...
- Latest tag: **`v0.1.535`** — `str(range(...))` now emits the Python range repr (`"range(0, 5)"`, `"range(2, 20, 3)"`): it materialized the range to a list and rendered `"[0, 1, 2, 3, 4]"` (silent-wrong). Recent: v0.1.534 zero-arg `int()`/`str()`/`float()` lower to `0`/`""`/`0.0`; v0.1.533 a nested-dict read-modify-write hoists the RHS before `&mut`; v0.1.532 a comprehension over a bare dict iterates its keys; v0.1.531 a bool with a format spec formats as an int; v0.1.530 `str(<dataclass>)` routes through the Display repr; v0.1.529 `d[True] = v` coerces the bool key to i64; v0.1.528 a dataclass field mutation in a loop uses `iter_mut`; v0.1.525 `map`/`filter`/`sum` over a homogeneous tuple materializes it; v0.1.520 method default args fill at the call site; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.508 a custom `__hash__` makes a dataclass a set/dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V26, ~1800 probes; 268+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
