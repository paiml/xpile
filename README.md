...
- Latest tag: **`v0.1.84`** — Python **filtered dict + set comprehensions `{… for … if cond}`**. Recent: v0.1.83 filtered list comprehension `[x for x in xs if cond]`, v0.1.82 dict get-or-insert `d.setdefault(k, default)`, v0.1.81 str padding `s.rjust(w)` / `s.ljust(w)`, v0.1.80 set removal. (v0.1.13 PTX gate.)
