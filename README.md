...
- Latest tag: **`v0.1.55`** — **context-aware `len(x)`** (works over `len(d.keys())`, `len(sorted(xs))`, …). Recent: v0.1.54 dict views `d.keys()/.values()`, v0.1.53 list query `xs.count/.index`, v0.1.52 reverse-slice `xs[::-1]`, v0.1.51 negative list indexing `xs[-1]`. (v0.1.13 PTX gate.)
