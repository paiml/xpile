...
- Latest tag: **`v0.1.524`** — two sibling nested `range()` loops reusing the same inner loop-var name now compile: the inner `let mut j` was scoped to the first outer loop, so the second nest's `j = …` was out of scope (E0425); a range loop var is now re-declared (shadowed) when reused across sibling loops. Recent: v0.1.523 a `@staticmethod` named a Rust keyword is `r#`-escaped at the call site; v0.1.522 `str.format()` float align/width keeps the trailing `.0`; v0.1.521 an unannotated param's type comes from its default; v0.1.520 method/staticmethod default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.518 `x if x is not None else d` narrows `x`; v0.1.517 `except E as e:` binds the message; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to dunders; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V24, ~1600 probes; 257+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
