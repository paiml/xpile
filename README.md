...
- Latest tag: **`v0.1.288`** — correctness: `int(1e30)` (an out-of-i64-range float) now fails loud instead of silently saturating to `i64::MAX` — completes the int-cast fail-loud story (non-finite + out-of-range). Recent: v0.1.287 a reused non-Copy call arg (`helper(xs) + helper(xs)`) clones instead of E0382, v0.1.286 a class/enum named after a prelude type is rejected cleanly (was E0107), v0.1.285 `int(inf)`/`int(nan)` panic, v0.1.284 a non-Copy field returned from `&self` clones (was E0507), v0.1.283 float `sum()` uses Neumaier compensation. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite + out-of-range, prelude-type-name reject).
