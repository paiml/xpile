...
- Latest tag: **`v0.1.266`** — correctness: str slicing indexes by Unicode char not byte (`"αβγδ"[1:3]`). Recent: v0.1.265 `str.find/index` return char index, v0.1.264 `bool` is an `int` subtype, v0.1.263 `len(str)` counts Unicode chars, v0.1.262 multiple `if` filters in comprehensions, v0.1.261 three-way `zip`. (v0.1.13 PTX gate.) The str Unicode trio (len/find/slice) + bool-as-int came from an adversarial differential python3-vs-rustc hunt.
