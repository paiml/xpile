...
- Latest tag: **`v0.1.210`** — **dataclass `@property`** (read-only computed attribute; `r.area` → `(r).area()`). Recent: v0.1.209 augmented struct field assignment, v0.1.208 dataclass `@classmethod`, v0.1.207 dataclass `@staticmethod`, v0.1.206 dataclass field defaults, v0.1.205 dataclass keyword construction. (v0.1.13 PTX gate.)
