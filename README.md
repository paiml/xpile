...
- Latest tag: **`v0.1.173`** — **bare callable as `key=`** for `min`/`max`/`sorted` (`key=abs`, `key=len`, `key=fn`). Recent: v0.1.172 `d.setdefault(k, v)` as a bare statement, v0.1.171 `xs.remove(x)` list remove-by-value, v0.1.170 `float` f-string field → Python repr `3.0`, v0.1.169 `bool` f-string field → `True`/`False`, v0.1.168 f-string lone `{n}` field + integer radix/width specs. (v0.1.13 PTX gate.)
