...
- Latest tag: **`v0.1.163`** — **nested subscript assignment** (`grid[i][j] = v`). Recent: v0.1.162 mixed `{**a, "k": v}` dict literals, v0.1.161 `{**d1, **d2}` dict merge, v0.1.160 expression-position set/dict comprehensions, v0.1.159 expression-position list comprehensions, v0.1.158 block expressions + multi-statement nested functions. (v0.1.13 PTX gate.)
