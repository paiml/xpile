...
- Latest tag: **`v0.1.65`** — Python **string predicates** `.isdigit()` `.isalpha()` `.isspace()` → `Bool`. Recent: v0.1.64 `str(float)` (completes `str()`), v0.1.63 `str(bool)`, v0.1.62 `str(int)`, v0.1.61 `map(lambda, xs)`. (v0.1.13 PTX gate.)
