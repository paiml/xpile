...
- Latest GitHub tag: **`v0.1.546`** — see [`CHANGELOG.md`](CHANGELOG.md) for the full per-tag history (one entry per slice). The recent cadence is a sustained run of correctness fixes from multi-pass adversarial differential `python3`-vs-`rustc` hunts (HUNT-V1…V26, ~1800 probes) that bring the transpiled Rust into exact agreement with CPython across an expanding Python subset.
