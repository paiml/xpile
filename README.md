...
- Latest tag: **`v0.1.194`** — **two-generator dict & set comprehensions** (`{k:v for x in a for y in b}` / `{e for …}` → nested `for` loops). Recent: v0.1.193 two-generator list comprehension, v0.1.192 bitwise invert `~x`, v0.1.191 Optional intra-branch narrowing (`if x is not None:`), v0.1.190 Optional flow-narrowing (cut 4 keystone), v0.1.189 1-arg `d.get(k)` → `Optional[V]`. (v0.1.13 PTX gate.)
