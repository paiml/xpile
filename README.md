...
- Published: [`xpile 0.1.7`](https://crates.io/crates/xpile) — adds Python **dict iteration** `for k in d:` (→ `for k in d.keys().cloned()`), completing the dict lane (read/write/`.get`/`in`/`len` shipped in v0.1.2). (v0.1.6 added cross-function return-type inference; v0.1.5 augmented assignment; v0.1.4 iterative C; v0.1.3 the C frontend.) All 27 workspace crates at 0.1.7; `cargo install xpile` upgrades cleanly.
