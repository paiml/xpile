...
- Latest tag: **`v0.1.285`** — correctness: `int(inf)`/`int(nan)` now panic (Python OverflowError/ValueError) instead of silently saturating/zeroing. Recent: v0.1.284 a method/`@property` returning a non-Copy field (`return self.name`) clones instead of E0507, v0.1.283 float `sum()` uses CPython 3.12+ Neumaier compensated summation, v0.1.282 float `str`/`repr`/`print` uses CPython scientific notation, v0.1.281 `repr(x)` works (was E0423/rejected), v0.1.280 float division by zero raises ZeroDivisionError. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, clone-field-read, int-cast non-finite).
