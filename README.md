...
- Latest tag: **`v0.1.231`** — **tuple-target genexpr/comprehension** (`sum(v for k, v in d.items())`, `sum(x*y for x, y in zip(a, b))`). Recent: v0.1.230 `str` reverse-slice `s[::-1]`, v0.1.229 bare `dict.pop()` statement, v0.1.228 bare `xs.pop()` statement, v0.1.227 container truthiness, v0.1.226 `map`/`filter` typed lambda. (v0.1.13 PTX gate.)
