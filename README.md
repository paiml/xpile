...
- Latest tag: **`v0.1.160`** — **expression-position set/dict comprehensions** (`len({x for x in xs})`). Recent: v0.1.159 expression-position list comprehensions, v0.1.158 block expressions + multi-statement nested functions, v0.1.157 `f(*xs)` splat into a variadic param, v0.1.156 nested functions, v0.1.155 varargs `*args`. (v0.1.13 PTX gate.)
