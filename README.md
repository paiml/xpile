...
- Latest tag: **`v0.1.201`** — **`@dataclass` → Rust struct definition** (classes epic, first cut: field-only class → `#[derive(Clone, Debug, PartialEq)] pub struct`; construction + field access follow). Recent: v0.1.200 statement-position assignment-form `try`/`except`, v0.1.199 `try`/`except` value-with-fallback via `catch_unwind`, v0.1.198 reject `tuple(<iterable>)` cleanly, v0.1.197 R6 COMPLETE (`C-XLATE-PY-DICT-TO-HASHMAP`), v0.1.196 `C-C-INT-ARITH` contract. (v0.1.13 PTX gate.)
