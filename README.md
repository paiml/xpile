...
- Latest tag: **`v0.1.199`** — **`try`/`except` (value-with-fallback)** via `catch_unwind` (xpile models Python exceptions as Rust panics; `try: return x except E: return y` catches ZeroDivisionError/IndexError/KeyError). Recent: v0.1.198 reject `tuple(<iterable>)` cleanly, v0.1.197 R6 COMPLETE (`C-XLATE-PY-DICT-TO-HASHMAP`), v0.1.196 `C-C-INT-ARITH` contract, v0.1.195 R6 grandfather the Diamond depth-13 gate, v0.1.194 two-generator dict & set comprehensions. (v0.1.13 PTX gate.)
