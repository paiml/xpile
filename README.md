...
- Latest tag: **`v0.1.23`** — Python **`enumerate` / `zip`** in `for` loops (`Stmt::ForEachPair` → `for (i, x) in xs.iter().cloned().enumerate()…` / `.zip(…)`). Recent: v0.1.22 DiffExecEngine hook, v0.1.21 slicing, v0.1.20 tuples, v0.1.18 string-method family. (v0.1.13 PTX gate.)
