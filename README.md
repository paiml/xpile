...
- Latest tag: **`v0.1.83`** — Python **filtered list comprehension `[x for x in xs if cond]`**. Recent: v0.1.82 dict get-or-insert `d.setdefault(k, default)`, v0.1.81 str padding `s.rjust(w)` / `s.ljust(w)`, v0.1.80 set removal `s.remove(x)` / `s.discard(x)`, v0.1.79 dict pop `d.pop(k)`. (v0.1.13 PTX gate.)
