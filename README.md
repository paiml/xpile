...
- Latest tag: **`v0.1.283`** — correctness: float `sum()` uses CPython 3.12+ Neumaier compensated summation (`sum([1,1e16,1,-1e16])` == 2.0, not 0.0); was naive. Recent: v0.1.282 float `str`/`repr`/`print` uses CPython scientific notation above the exp threshold, v0.1.281 `repr(x)` works (was E0423/rejected), v0.1.280 float division by zero raises ZeroDivisionError, v0.1.279 `bool & | ^ bool` stays a bool, v0.1.278 `abs()` of an i64 overflow-checks (`abs(i64::MIN)` panics). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation).
