...
- Latest tag: **`v0.1.85`** — Python **list comprehension over `range(...)`** `[x*x for x in range(n)]`. Recent: v0.1.84 filtered dict + set comprehensions, v0.1.83 filtered list comprehension, v0.1.82 dict get-or-insert `d.setdefault(k, default)`, v0.1.81 str padding `s.rjust(w)` / `s.ljust(w)`. (v0.1.13 PTX gate.)
