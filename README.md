...
- Latest tag: **`v0.1.207`** — **dataclass `@staticmethod`** (`Class.method(args)` → `Class::method(args)`, a no-`self` associated fn). Recent: v0.1.206 dataclass field defaults, v0.1.205 dataclass keyword construction, v0.1.204 dataclass methods, v0.1.203 dataclass field assignment, v0.1.202 dataclass construction + field access (`Type::Struct`). (v0.1.13 PTX gate.)
