...
- Published: [`xpile 0.1.9`](https://crates.io/crates/xpile) — adds Python **keyword arguments** in calls (`f(x=1, y=2)`), reordered to positional at lowering via the callee’s declared parameter order. (v0.1.8 list comprehensions; v0.1.7 dict iteration; v0.1.6 cross-function inference; v0.1.5 augmented assignment; v0.1.4 iterative C; v0.1.3 the C frontend.) All 27 workspace crates at 0.1.9; `cargo install xpile` upgrades cleanly.
