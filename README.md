...
- Latest tag: **`v0.1.281`** — correctness: `repr(x)` now works (was rustc E0423 / rejected) — `repr(str)` emits CPython-style quotes+escapes, `repr(int/float/bool)` reuses `str()`. Recent: v0.1.280 float division by zero raises ZeroDivisionError like Python (was `inf`/`nan`), v0.1.279 `bool & | ^ bool` stays a bool, v0.1.278 `abs()` of an i64 overflow-checks (`abs(i64::MIN)` panics), v0.1.277 `sorted()` over a `list[float]` compiles (was E0277), v0.1.276 `x >> n` for `n ≥ 64` saturates to the sign fill. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr()).
