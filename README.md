...
- Latest tag: **`v0.1.515`** — a unary operator over a custom class now dispatches to its dunder: `-obj` → `obj.__neg__()`, `~obj` → `obj.__invert__()` (previously rejected as "requires an I64 operand"), the unary analog of the binop dunder cluster. Recent: v0.1.514 `dict(d)` over a dict is an independent copy; v0.1.513 a method named a Rust keyword (`def type(self)`) is `r#`-escaped at def + call site; v0.1.512 `set("abc")`/`set((1,2,3))` materialize str-chars / tuple-elements; v0.1.511 the `dict(a=1, b=2)` keyword constructor lowers to a string-keyed dict literal; v0.1.510 a dataclass field named a Rust keyword is `r#`-escaped (label-stripped in the repr); v0.1.509 a walrus-bound name reassigned once gets `let mut`; v0.1.508 a dataclass with a custom `__hash__` is usable as a set element / dict key; v0.1.507 a float with an align/width-only spec keeps Python's trailing `.0`; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V23, ~1500 probes; 248+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
