...
- Latest tag: **`v0.1.190`** — **Optional flow-narrowing** (Optional epic cut 4, the keystone: after `if x is None: return …`, a later read of `x` unwraps to `T` so guard-then-use compiles). Recent: v0.1.189 1-arg `d.get(k)` → `Optional[V]`, v0.1.188 Optional params + `is None` tests, v0.1.187 `Optional[T]` return type, v0.1.186 `sorted(s)` over a str, v0.1.185 `sorted(d)` over a dict. (v0.1.13 PTX gate.)
