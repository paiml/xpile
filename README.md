...
- Latest tag: **`v0.1.225`** — **comprehension/genexpr over typed elements** (`[p[1] for p in pairs]`, `sum(p.x for p in pts)`; fixes a silent miscompile/reject). Recent: v0.1.224 `sorted`/`min`/`max` tuple key, v0.1.223 negative-step `range`, v0.1.222 `len`/`sorted`/`reversed`(range) + `list(dict)`, v0.1.221 `sum`/`max`/`min` over range/set, v0.1.220 `list(set)`/`sorted(set)`. (v0.1.13 PTX gate.)
