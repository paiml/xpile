...
- Latest tag: **`v0.1.81`** — Python **string padding `s.rjust(w)` / `s.ljust(w)`**. Recent: v0.1.80 set removal `s.remove(x)` / `s.discard(x)`, v0.1.79 dict pop `d.pop(k)`, v0.1.78 item deletion `del coll[key]`, v0.1.77 list `xs.pop()`. (v0.1.13 PTX gate.)
