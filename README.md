...
- Latest tag: **`v0.1.258`** — subscript-target **swap** `xs[i], xs[j] = xs[j], xs[i]`. Recent: v0.1.257 f-string percent `f"{x:.1%}"`, v0.1.256 f-string sign flag `f"{x:+}"`, v0.1.255 expr-position 2-generator genexpr/comprehension, v0.1.254 in-place `xs.sort(reverse=True)`, v0.1.253 `math.perm(n, k)`. (v0.1.13 PTX gate.)
