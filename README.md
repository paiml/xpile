...
- Latest tag: **`v0.1.291`** — correctness: a `@dataclass(frozen=True)` used as a dict key / set element now derives `Eq` + `Hash` (when all fields are Eq+Hash-capable), instead of emitting a bare `#[derive(Clone, Debug, PartialEq)]` struct that rustc rejects as a `HashMap` key / `HashSet` element (E0277/E0599). Recent: v0.1.290 float `%` matches CPython `float_rem` bit-exactly (was last-ULP off + lost signed zero), v0.1.289 `xs.insert(i, x)` clamps out-of-range / negative indices to CPython `ins1` semantics instead of panicking, v0.1.288 `int(1e30)` (out-of-i64-range float) fails loud instead of silently saturating, v0.1.287 a reused non-Copy call arg (`helper(xs) + helper(xs)`) clones instead of E0382, v0.1.286 a class/enum named after a prelude type is rejected cleanly (was E0107). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite + out-of-range, prelude-type-name reject, list.insert index clamp, float-mod fmod parity, frozen-dataclass Eq+Hash).
