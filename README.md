...
- Latest tag: **`v0.1.286`** — correctness: a class/enum named after a prelude type xpile emits (`Vec`/`String`/`Option`/`HashMap`/…) is now rejected cleanly instead of emitting colliding Rust (E0107). Recent: v0.1.285 `int(inf)`/`int(nan)` panic instead of silently saturating, v0.1.284 a non-Copy field returned from `&self` clones instead of E0507, v0.1.283 float `sum()` uses CPython 3.12+ Neumaier compensation, v0.1.282 float scientific notation, v0.1.281 `repr(x)` works (was E0423/rejected). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, clone-field-read, int-cast non-finite, prelude-type-name reject).
