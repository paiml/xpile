...
- Published: [`xpile 0.1.12`](https://crates.io/crates/xpile) — adds **early returns** (guard clauses): a non-final `return` inside an `if` branch (`Stmt::Return`) — recursive `fact` via a guard clause and 3-way `sign` round-trip through rustc. Completes the post-v0.1.4 EV ladder R1–R5, R7–R10. (v0.1.11 C if/else; v0.1.10 float; v0.1.9 kwargs; v0.1.8 comprehensions; v0.1.7 dict iteration.) All 27 workspace crates at 0.1.12; `cargo install xpile` upgrades cleanly.
