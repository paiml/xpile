...
- Latest tag: **`v0.1.26`** — Python **numeric builtins** `abs`/`min`/`max` → `(x).abs()` / `(a).min(b)` / `(a).max(b)` (`Expr::NumBuiltin`). Recent: v0.1.25 aug-subscript-assign, v0.1.24 structured `via_roles`, v0.1.23 enumerate/zip, v0.1.21 slicing. (v0.1.13 PTX gate.)
