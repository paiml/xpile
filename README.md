...
- Published: [`xpile 0.1.10`](https://crates.io/crates/xpile) — adds the Python **`float`** type (f64): float params/returns/locals, plain-infix IEEE-754 arithmetic, and true division (`/`). xpile’s first non-integer numeric type. (v0.1.9 keyword args; v0.1.8 list comprehensions; v0.1.7 dict iteration; v0.1.6 cross-function inference; v0.1.5 augmented assignment; v0.1.3/1.4 the C frontend.) All 27 workspace crates at 0.1.10; `cargo install xpile` upgrades cleanly.
