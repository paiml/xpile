...
- Latest tag: **`v0.1.75`** — Python **list `xs.extend(ys)`** (in-place concatenation). Recent: v0.1.74 in-place list mutators `xs.sort()`/`.reverse()`/`.clear()`, v0.1.73 assert with message, v0.1.72 list membership `x in xs`, v0.1.71 f-string format specs. (v0.1.13 PTX gate.)
