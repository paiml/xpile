...
- Latest tag: **`v0.1.224`** — **`sorted`/`min`/`max` key indexing a tuple** (`key=lambda p: p[1]` → `.1`; fixes a silent miscompile). Recent: v0.1.223 negative-step `range`, v0.1.222 `len`/`sorted`/`reversed`(range) + `list(dict)`, v0.1.221 `sum`/`max`/`min` over range/set, v0.1.220 `list(set)`/`sorted(set)`, v0.1.219 `frozenset`. (v0.1.13 PTX gate.)
