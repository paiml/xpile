...
- Latest tag: **`v0.1.30`** — Python **dict comprehensions** `{k: v for x in xs}` (materialise to `acc = {}` + `for x in xs: acc[k] = v`; no new IR). Recent: v0.1.29 sets, v0.1.28 general `if/else`, v0.1.27 `sum`, v0.1.26 abs/min/max. (v0.1.13 PTX gate.)
