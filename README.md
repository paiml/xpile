...
- Latest tag: **`v0.1.168`** — **f-string lone `{n}` field + integer radix/width specs** (`:x`/`:b`/`:o`/`:05`/`:08b`). Recent: v0.1.167 empty list literal `[]` annotation threading, v0.1.166 `xs += ys` list in-place extend, v0.1.165 nested augmented subscript assignment (`grid[i][j] += v`) + latent `let mut` fix, v0.1.164 `for _ in range(n)` underscore loop targets, v0.1.163 nested subscript assignment (`grid[i][j] = v`). (v0.1.13 PTX gate.)
