...
- Latest tag: **`v0.1.242`** — **two-generator comprehensions over `range`** (`[i*j for i in range(n) for j in range(m)]`). Recent: v0.1.241 mixed `float`/`int` ternary, v0.1.240 mixed-numeric `min()`/`max()`, v0.1.239 mixed `float`/`int` compare + arithmetic, v0.1.238 negative/OOB slice bounds, v0.1.237 `//`/`%` with a negative divisor. (v0.1.13 PTX gate.)
