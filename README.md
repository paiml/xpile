...
- Latest tag: **`v0.1.533`** — a nested-dict read-modify-write (`d["a"]["x"] = d["a"]["x"] + 5`, the 2D-dict / Counter accumulation idiom) now compiles: the RHS read the dict while `&mut d` was live (E0502); the RHS is now hoisted into a temp first. Recent: v0.1.532 a comprehension over a bare dict iterates its keys; v0.1.531 a bool with a format spec formats as an int; v0.1.530 `str(<dataclass>)` routes through the Display repr; v0.1.529 `d[True] = v` coerces the bool key to i64; v0.1.528 a dataclass field mutation in a loop uses `iter_mut`; v0.1.527 a reused try-body call arg is cloned; v0.1.526 a dict comprehension `{w[0]: w}` clones the bare-binder value; v0.1.525 `map`/`filter`/`sum` over a homogeneous tuple materializes it; v0.1.520 method default args fill at the call site; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.508 a custom `__hash__` makes a dataclass a set/dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V26, ~1800 probes; 266+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
