...
- Latest tag: **`v0.1.272`** — correctness: Rust-keyword identifiers (`type`/`match`/`loop`/`move` as Python var/param/fn names → `r#name`). Recent: v0.1.271 tuple reassignment in a loop/if body (`a, b = b, a % b` — Euclid GCD / Fibonacci), v0.1.270 3-arg `pow(a, b, m)` modpow, v0.1.269 negative `xs.pop(-k)`/`del xs[-k]`, v0.1.268 list-of-list repeat `[[0]] * n`, v0.1.267 `max(key=)`/`sorted(reverse=)` tie+stability, v0.1.266 str slicing by Unicode char. (v0.1.13 PTX gate.) A run of recent correctness fixes came from a two-pass adversarial differential python3-vs-rustc hunt (str Unicode, bool-as-int, tie/stability, list-repeat, neg pop/del, modpow, loop-var reassignment, keyword identifiers).
