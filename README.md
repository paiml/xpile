...
- Latest tag: **`v0.1.59`** — Python **`min/max(xs, key=lambda)`** → `min_by_key`/`max_by_key`. Recent: v0.1.58 `sorted(key=lambda)` (first lambda support), v0.1.57 `for k,v in d.items()`, v0.1.56 `d.items()`, v0.1.55 context-aware `len(x)`. (v0.1.13 PTX gate.)
