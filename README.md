...
- Latest tag: **`v0.1.188`** — **Optional params + `is None`/`is not None` tests** (Optional epic cut 2+3: `Optional[T]` param → `Option<T>`, `x is None` → `x.is_none()`). Recent: v0.1.187 `Optional[T]` return type, v0.1.186 `sorted(s)` over a str, v0.1.185 `sorted(d)` over a dict, v0.1.184 set splat literals (`{*a, *b}`), v0.1.183 list splat literals (`[*a, *b]`). (v0.1.13 PTX gate.)
