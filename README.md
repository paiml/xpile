...
- Latest tag: **`v0.1.270`** — 3-arg `pow(a, b, m)` modular exponentiation. Recent: v0.1.269 negative `xs.pop(-k)`/`del xs[-k]`, v0.1.268 list-of-list repeat `[[0]] * n` compiles, v0.1.267 `max(key=)`/`sorted(reverse=)` tie+stability, v0.1.266 str slicing by Unicode char, v0.1.265 `str.find/index` char index. (v0.1.13 PTX gate.) A run of recent correctness fixes (str Unicode, bool-as-int, tie/stability, list-repeat, neg pop/del, modpow) came from an adversarial differential python3-vs-rustc hunt.
