...
- Latest tag: **`v0.1.42`** — Python **`all(xs)`/`any(xs)`** over `list[bool]` (`Expr::BoolReduce`). Recent: v0.1.41 empty constructors `set()/dict()/list()`, v0.1.40 `min/max` over `list[float]`, v0.1.39 set algebra `a|b a&b a-b a^b`, v0.1.38 `sorted(reverse=True)`. (v0.1.13 PTX gate.)
