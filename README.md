...
- Latest tag: **`v0.1.90`** — Python **multi-parameter + nullary closures** (`f = lambda x, y: x+y`). Recent: v0.1.89 first-class closures, v0.1.88 dict + set comprehensions over `range(...)`, v0.1.87 general slice step `xs[a:b:c]`, v0.1.86 in-place dict merge `a.update(b)`. (v0.1.13 PTX gate.)
