...
- Latest tag: **`v0.1.509`** — a walrus-bound name reassigned once (`if (n := 10) > 5: n = 99`) now compiles: the walrus binds `n` in the if-condition expression, which the mut-inference missed, so the binding emitted a non-`mut` `let n` and the reassignment was rustc E0384; the condition is now scanned for walrus targets, so a reassigned walrus binding becomes `let mut` (a never-reassigned one stays plain `let`). Recent: v0.1.508 a dataclass with a custom `__hash__` is usable as a set element / dict key (derives `Eq` + delegating `impl Hash`); v0.1.507 a float with an align/width-only spec keeps Python's trailing `.0`; v0.1.506 a bare `range(...)` bound to a variable materializes to a `Vec<i64>`; v0.1.505 comprehension tuple loop vars no longer pollute function scope; v0.1.503 `except LookupError`/`ArithmeticError` catch only their member exceptions; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V22, ~1400 probes; 242+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
