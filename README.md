...
- Latest tag: **`v0.1.275`** — correctness: a chained comparison (`0 < xs.pop() < 100`) now evaluates each operand exactly once (was double-evaluated → wrong result / double side-effect). Recent: v0.1.274 left-shift value overflow (`1 << 63`) panics per C-PY-INT-ARITH instead of silently wrapping, v0.1.273 mutating method (`xs.pop()`) in a `while`/`if`/`assert` condition marks its receiver `mut` (was rustc E0596), v0.1.272 Rust-keyword identifiers (`type`/`match`/`loop`/`move` → `r#name`), v0.1.271 tuple reassignment in a loop/if body (`a, b = b, a % b` — Euclid GCD / Fibonacci), v0.1.270 3-arg `pow(a, b, m)` modpow. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, list-repeat, modpow, loop-var reassignment, keyword identifiers, mut-receiver-in-condition, left-shift overflow, chained-compare double-eval).
