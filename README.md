...
- Latest tag: **`v0.1.292`** — correctness: PEP 584 dict union `a | b` (new dict, `b` wins on key collisions) and `a |= b` (in-place update) are now lowered (reusing the `{**a, **b}` / `a.update(b)` machinery) instead of emitting `HashMap | HashMap` (rustc E0369); other dict binary operators are rejected cleanly. Recent: v0.1.291 a `@dataclass(frozen=True)` used as a dict key / set element derives `Eq` + `Hash` (was E0277/E0599), v0.1.290 float `%` matches CPython `float_rem` bit-exactly (was last-ULP off + lost signed zero), v0.1.289 `xs.insert(i, x)` clamps out-of-range / negative indices to CPython `ins1` semantics instead of panicking, v0.1.288 `int(1e30)` (out-of-i64-range float) fails loud instead of silently saturating, v0.1.287 a reused non-Copy call arg (`helper(xs) + helper(xs)`) clones instead of E0382. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite + out-of-range, prelude-type-name reject, list.insert index clamp, float-mod fmod parity, frozen-dataclass Eq+Hash, PEP 584 dict union).
