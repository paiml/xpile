...
- Latest tag: **`v0.1.220`** — **`list(set(…))` / `sorted(set(…))`** → materialise a set to a `Vec` (fixes a silent miscompile). Recent: v0.1.219 `frozenset`, v0.1.218 `str.split` maxsplit, v0.1.217 `str.replace` 3-arg, v0.1.216 `str.startswith`/`endswith` tuple, v0.1.215 enum `.name`. (v0.1.13 PTX gate.)
