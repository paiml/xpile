...
- Latest tag: **`v0.1.29`** — Python **sets** (read-side): literal `{a,b,c}` → `HashSet`, `x in s` → `s.contains(&x)` (`Type::Set`/`Expr::SetLit`/`SetContains`). Recent: v0.1.28 general `if/else`, v0.1.27 `sum`, v0.1.26 abs/min/max, v0.1.25 aug-subscript-assign. (v0.1.13 PTX gate.)
