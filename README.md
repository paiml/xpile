...
- Latest tag: **`v0.1.530`** — `str(<dataclass>)` now works: it had no `str()` arm so it mis-inferred as i64 (a function `-> str: return str(p)` was rejected); it now routes through the dataclass Display repr (the same `format!("{:}", p)` an f-string uses). Recent: v0.1.529 `d[True] = v` coerces the bool key to i64; v0.1.528 a dataclass field mutation in a loop uses `iter_mut`; v0.1.527 a reused try-body call arg is cloned; v0.1.526 a dict comprehension `{w[0]: w}` clones the bare-binder value; v0.1.525 `map`/`filter`/`sum` over a homogeneous tuple materializes it; v0.1.522 `str.format()` float align keeps the `.0`; v0.1.521 an unannotated param's type comes from its default; v0.1.520 method default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.508 a custom `__hash__` makes a dataclass a set/dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V25, ~1700 probes; 263+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
