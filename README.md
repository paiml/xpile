...
- Latest tag: **`v0.1.232`** — **in-place set/dict mutators** (`s.update(t)`, `s.clear()`, `d.clear()`). Recent: v0.1.231 tuple-target genexpr/comprehension, v0.1.230 `str` reverse-slice `s[::-1]`, v0.1.229 bare `dict.pop()` statement, v0.1.228 bare `xs.pop()` statement, v0.1.227 container truthiness. (v0.1.13 PTX gate.)
