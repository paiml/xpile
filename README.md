...
- Latest tag: **`v0.1.171`** — **`xs.remove(x)` list remove-by-value** (completes the in-place list-mutator surface). Recent: v0.1.170 `float` f-string field → Python repr `3.0`, v0.1.169 `bool` f-string field → `True`/`False`, v0.1.168 f-string lone `{n}` field + integer radix/width specs, v0.1.167 empty list literal `[]` annotation threading, v0.1.166 `xs += ys` list in-place extend. (v0.1.13 PTX gate.)
