...
- Latest tag: **`v0.1.247`** — **negative-step list slice** (`xs[::-k]`). Recent: v0.1.246 tuple-unpack init + augment, v0.1.245 comprehensions/genexprs over a string, v0.1.244 `str.rfind`/`str.rindex`, v0.1.243 `enumerate()`/`zip()` over a string, v0.1.242 two-generator comprehensions over `range`. (v0.1.13 PTX gate.)
