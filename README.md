...
- Latest tag: **`v0.1.22`** — §29 **`DiffExecEngine` interface + hook** (Track 4): the Multi-Emitter Quorum's Runtime-stratum trait; no engine → benign `NotRun`, engine error → hard failure. Real CUDA/Vulkan engines plug in out-of-band. Recent capability: v0.1.21 slicing, v0.1.20 tuples, v0.1.18 string methods. (v0.1.13 PTX gate.)
