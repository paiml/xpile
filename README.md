...
- Latest tag: **`v0.1.140`** — context-aware **ternary branches** (builtins in `x if c else y`). Recent: v0.1.139 `int(s, base)` radix parsing, v0.1.138 variadic `min`/`max` (`max(a, b, c)`), v0.1.137 `pow(a, b)` builtin, v0.1.136 `sum(xs, start)` 2-arg form, v0.1.135 `set(xs)` materialization. (v0.1.13 PTX gate.)
