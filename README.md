...
- Latest tag: **`v0.1.235`** — **`int(b)` / `float(b)` over a `bool`** (enables `sum(int(b) for b in bs)`). Recent: v0.1.234 `x in range(...)` membership, v0.1.233 in-place append on a subscript receiver, v0.1.232 in-place set/dict mutators, v0.1.231 tuple-target genexpr/comprehension, v0.1.230 `str` reverse-slice `s[::-1]`. (v0.1.13 PTX gate.)
