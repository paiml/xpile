...
- Published: [`xpile 0.1.6`](https://crates.io/crates/xpile) — adds **cross-function return-type inference**: a module signature table types `s = make_scores()` as the callee's declared `dict[str,int]` return (was a hardcoded `i64` fallback that silently broke `s["alice"]`), unblocking multi-function programs that compose dict/list/str helpers. (v0.1.5 added Python augmented assignment; v0.1.4 iterative C; v0.1.3 the C frontend; v0.1.2 dict operations.) All 27 workspace crates at 0.1.6; `cargo install xpile` upgrades cleanly.
