...
- Latest tag: **`v0.1.505`** — a comprehension's tuple loop vars (`k, v` in `{k: v*2 for k, v in d.items()}`) no longer pollute function scope: a later same-named real `for k` loop emitted a bare `k = __fe0` to a never-`let` name (rustc E0425) because the comp registered `k` as a durable binding; comp tuple vars are now loop-scoped, so the later loop binds a fresh `for k`. Recent: v0.1.504 a bool needle in an int-set membership test (`True in {1, 2, 3}`) coerces to i64; v0.1.503 `except LookupError:`/`except ArithmeticError:` catch only their member exceptions (not a silent catch-all); v0.1.502 `x in d` with a needle type that can't match the key type folds to `False`; v0.1.501 a string index out of range raises a catchable `IndexError`; v0.1.500 plain-width radix of a negative int (`f"{-255:8x}"`) is sign-magnitude not two's-complement **(500th-patch milestone — 233+ consecutive differential correctness fixes, HUNT-V14…V19, bringing transpiled Rust into exact agreement with CPython)**; v0.1.499 `continue` inside `for i in range(n)` compiles; v0.1.498 an N-arity (≥3) tuple `for`-target desugars to a tuple-unpack; v0.1.497 `d[k].pop()` mutates in place (was a clone); v0.1.496 `list/str * bool` coerces the count; v0.1.495 `abs/round(bool)`; v0.1.494 `math.sqrt`/`log` domain → catchable `ValueError`; v0.1.493 `int(±inf)`/`int(nan)` → catchable `OverflowError`/`ValueError`; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V19, ~1100 probes).
