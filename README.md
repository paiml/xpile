...
- Latest tag: **`v0.1.197`** — **R6 COMPLETE: `C-XLATE-PY-DICT-TO-HASHMAP` authored** at depth-1 (the dict half). With `C-C-INT-ARITH` (v0.1.196), the §7.3 "every construct under contract" falsification is fully restored to true — 15 on-disk contracts, both R6 newcomers at depth-1 via the grandfathered gate. Recent: v0.1.196 `C-C-INT-ARITH` contract, v0.1.195 R6 grandfather the Diamond depth-13 gate, v0.1.194 two-generator dict & set comprehensions, v0.1.193 two-generator list comprehension, v0.1.192 bitwise invert `~x`. (v0.1.13 PTX gate.)
