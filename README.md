...
- Latest tag: **`v0.1.520`** — a method/staticmethod default argument now works: `def m(self, x=5)` then `c.m()` dropped the default and left the call under-supplied (rustc E0061); instance methods now register a signature (under a distinct key) and both instance and static call sites fill omitted trailing args with the defaults (coerced to the param type — Some-wrap / int→f64). Recent: v0.1.519 a struct list-index with `__index__` works; v0.1.518 the `x if x is not None else default` idiom narrows `x`; v0.1.517 `except E as e:` binds the exception message; v0.1.516 a for-loop mutating each element in place uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to dunders; v0.1.514 `dict(d)` is an independent copy; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.511 `dict(a=1, b=2)` → dict literal; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V24, ~1600 probes; 253+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
