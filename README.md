...
- Latest GitHub tag: **`v0.1.597`** — see [`CHANGELOG.md`](CHANGELOG.md) for the full per-tag history (one entry per slice). The recent cadence pairs a sustained run of correctness fixes from multi-pass adversarial differential `python3`-vs-`rustc` hunts (HUNT-V1…V33, ~2000+ probes) — bringing the transpiled Rust into exact agreement with CPython across an expanding Python subset — with the Sprint-2 turn to the two north-star differentiators: a differential **oracle** (auto-diffs CPython vs transpiled-Rust stdout in CI) and **Phase-5 hybrid** transpilation (the `xpile hybrid <dir>` command now dispatches a mixed-language module directory and reconciles its cross-language FFI boundaries end-to-end), all under the R6 contract-integrity regime so every emitted construct cites a provable contract.
