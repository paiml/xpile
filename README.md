...
- Latest tag: **`v0.1.265`** — correctness: `str.find/index` return a char index not a byte offset. Recent: v0.1.264 `bool` is an `int` subtype, v0.1.263 `len(str)` counts Unicode chars, v0.1.262 multiple `if` filters in comprehensions, v0.1.261 three-way `zip`, v0.1.260 in-place keyed sort `xs.sort(key=lambda)`. (v0.1.13 PTX gate.) Several recent correctness fixes came from an adversarial differential python3-vs-rustc hunt.
