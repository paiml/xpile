...
- Latest tag: **`v0.1.184`** — **set splat literals** (`{*a, *b}`, `{*a, x}`). Recent: v0.1.183 list splat literals (`[*a, *b]`), v0.1.182 `min(xs)`/`max(xs)` over a `list[str]`, v0.1.181 collection `.copy()` (`list`/`dict`/`set`), v0.1.180 set predicates (`issubset`/`issuperset`/`isdisjoint` + `<=`/`<`/`>=`/`>`), v0.1.179 set-algebra methods (`union`/`intersection`/`difference`/`symmetric_difference`). (v0.1.13 PTX gate.)
