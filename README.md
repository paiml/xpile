...
- Latest tag: **`v0.1.186`** — **`sorted(s)` over a str** (sorts characters; completes the `sorted(X)` family). Recent: v0.1.185 `sorted(d)` over a dict, v0.1.184 set splat literals (`{*a, *b}`), v0.1.183 list splat literals (`[*a, *b]`), v0.1.182 `min(xs)`/`max(xs)` over a `list[str]`, v0.1.181 collection `.copy()` (`list`/`dict`/`set`). (v0.1.13 PTX gate.)
