...
- Latest tag: **`v0.1.203`** — **dataclass field assignment** (`obj.field = v` → `(obj).field = v;`, receiver auto-`mut`). Recent: v0.1.202 dataclass construction + field access (`Type::Struct`), v0.1.201 `@dataclass` → struct definition, v0.1.200 statement-position assignment-form `try`/`except`, v0.1.199 `try`/`except` value-with-fallback via `catch_unwind`, v0.1.198 reject `tuple(<iterable>)` cleanly. (v0.1.13 PTX gate.)
