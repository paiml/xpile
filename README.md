...
- Latest tag: **`v0.1.517`** — `except E as e:` now binds the caught exception's message to `e` (previously rejected): the handler gets a `String` `e` holding xpile's panic-payload message, so `str(e)`/`f"{e}"` agree with CPython for the common types (e.g. ZeroDivisionError "integer division or modulo by zero"). Recent: v0.1.516 a for-loop mutating each element in place uses `iter_mut`; v0.1.515 unary `-obj`/`~obj` dispatch to `__neg__`/`__invert__`; v0.1.514 `dict(d)` is an independent copy; v0.1.513 a method named a Rust keyword is `r#`-escaped; v0.1.512 `set("abc")`/`set((1,2,3))` materialize str-chars / tuple-elements; v0.1.511 `dict(a=1, b=2)` → string-keyed dict literal; v0.1.510 a keyword dataclass field is `r#`-escaped; v0.1.508 a dataclass with a custom `__hash__` is a set element / dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V23, ~1500 probes; 250+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
