...
- Latest tag: **`v0.1.123`** — **for-loops over call iterables** (`reversed(xs)`, `sorted(xs)`, `list(range(n))`). Recent: v0.1.122 `list(range(...))` materialization, v0.1.121 `for i in reversed(range(...))`, v0.1.120 `str.format` with specs (`{:.2f}`, `{:05d}`), v0.1.119 list & set comprehensions over `d.items()`, v0.1.118 dict comprehension over `d.items()`. (v0.1.13 PTX gate.)
