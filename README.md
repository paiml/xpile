...
- Latest tag: **`v0.1.298`** — correctness: a dict comprehension reusing a non-Copy loop var in both key and value (`{w: w for w in words}`) now clones the key instead of moving it into the tuple before the value can use it (E0382). Recent: v0.1.297 an empty `set()` infers its element type from the subsequent `.add(...)` (was `HashSet<i64>` default E0308), v0.1.296 the standalone `format(value[, spec])` builtin (was a bare `format(...)` macro-call E0423), v0.1.295 `reversed(s)` over a `str` reverses its characters (was a bare `reversed(...)` E0425), v0.1.294 integer `sum()` / `enumerate(xs, start)` honor the C-PY-INT-ARITH overflow contract (was silent `-O` wrap), v0.1.293 `enumerate(xs, start=N)` keyword form honors the start (was silently `+0`). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation, float-sum compensation, the ownership clones field-read + call-arg-reuse, int-cast non-finite + out-of-range, prelude-type-name reject, list.insert index clamp, float-mod fmod parity, frozen-dataclass Eq+Hash, PEP 584 dict union, enumerate start= keyword, int sum/enumerate overflow contract, reversed(str), format() builtin, empty-set element inference, dict-comp key clone).
