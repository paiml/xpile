...
- Published: [`xpile 0.1.2`](https://crates.io/crates/xpile) — incremental release completing the Python **dict operations** lane (`d[k]` read, `d[k] = v` write, `d.get(k, default)`, `k in d`, `len(d)`, empty annotated `{}`, and `name: T = value` annotated locals) on top of v0.1.1's str/list/dict-foundation. Rust + Ruchy emit a real `HashMap` pipeline; Lean refuses dict ops (deferred to the `Std.HashMap` encoding). All 27 workspace crates at 0.1.2; `cargo install xpile` upgrades cleanly.
