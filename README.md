...
- Latest tag: **`v0.1.528`** — a dataclass field mutation in a loop (`for p in pts: p.x = …`) now compiles and propagates: it emitted `pts.iter().cloned()` (p not mut → E0594, the clone discarding the mutation); the `iter_mut` detection now covers attribute assignment + a struct element, and a local iterable is marked `mut`. Recent: v0.1.527 a reused try-body call arg is cloned (try/except read-counting); v0.1.526 a dict comprehension `{w[0]: w}` clones the bare-binder value; v0.1.525 `map`/`filter`/`sum` over a homogeneous tuple materializes it; v0.1.524 sibling nested `range()` loops reusing an inner var compile; v0.1.523 a `@staticmethod` named a Rust keyword is `r#`-escaped; v0.1.522 `str.format()` float align keeps the `.0`; v0.1.521 an unannotated param's type comes from its default; v0.1.520 method default args fill at the call site; v0.1.519 a struct list-index with `__index__` works; v0.1.516 a for-loop mutating each element uses `iter_mut`; v0.1.508 a custom `__hash__` makes a dataclass a set/dict key; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.471 the dunder-dispatch cluster. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V25, ~1700 probes; 261+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
