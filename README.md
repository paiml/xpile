...
- Latest tag: **`v0.1.280`** — correctness: float division by zero (`a / b`, `a // b`, `a % b`, int `a / 0`) now raises ZeroDivisionError like Python (was `inf`/`nan`). Recent: v0.1.279 `bool & | ^ bool` stays a bool (was rejected as int), v0.1.278 `abs()` of an i64 overflow-checks (`abs(i64::MIN)` panics), v0.1.277 `sorted()` over a `list[float]` compiles (was E0277), v0.1.276 `x >> n` for `n ≥ 64` saturates to the sign fill, v0.1.275 chained comparison (`0 < xs.pop() < 100`) evaluates each operand exactly once. (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero).
