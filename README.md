...
- Latest tag: **`v0.1.510`** — a dataclass field named after a Rust keyword (`type`, `match`, `ref`) now compiles: it emitted `pub type: i64` / `(e).type` verbatim → a rustc keyword parse error; the reserved-ident escape now `r#`-escapes struct field names, field accesses, and struct-literal keys, while the Display field-repr label strips the `r#` back so the repr shows the Python name. Recent: v0.1.509 a walrus-bound name reassigned once gets `let mut`; v0.1.508 a dataclass with a custom `__hash__` is usable as a set element / dict key; v0.1.507 a float with an align/width-only spec keeps Python's trailing `.0`; v0.1.506 a bare `range(...)` bound to a variable materializes to a `Vec<i64>`; v0.1.505 comprehension tuple loop vars no longer pollute function scope; v0.1.503 `except LookupError`/`ArithmeticError` catch only their member exceptions; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V22, ~1400 probes; 242+ consecutive fixes bringing transpiled Rust into exact agreement with CPython).
