...
- Latest tag: **`v0.1.193`** — **two-generator list comprehension** (`[expr for x in a for y in b]` → nested `for` loops). Recent: v0.1.192 bitwise invert `~x`, v0.1.191 Optional intra-branch narrowing (`if x is not None:`), v0.1.190 Optional flow-narrowing (cut 4 keystone), v0.1.189 1-arg `d.get(k)` → `Optional[V]`, v0.1.188 Optional params + `is None` tests. (v0.1.13 PTX gate.)
