...
- Published: [`xpile 0.1.3`](https://crates.io/crates/xpile) — incremental release adding **xpile's second source language**: a real `decy` C → Rust frontend for the stack-only int subset (`int` functions, params, local decls, recursion, ternary, `+ - *`, comparisons, `&& ||`). C lowers with **C arithmetic semantics** — `i32` + `wrapping_*` (signed-overflow-as-UB), distinct from Python's `i64` + `checked_*`/bigint — via an isolated codegen path. `int factorial(int)` round-trips through rustc and computes correctly. (v0.1.2 completed the Python **dict operations** lane: `d[k]`, `d[k]=v`, `d.get`, `k in d`, `len(d)`, empty `{}`, annotated locals.) All 27 workspace crates at 0.1.3; `cargo install xpile` upgrades cleanly.
