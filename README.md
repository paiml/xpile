...
- Latest tag: **`v0.1.282`** — correctness: float `str`/`repr`/`print` now uses CPython scientific notation above the exp `< -4` / `>= 16` threshold (`1e16` → `1e+16`); was spelled out. Recent: v0.1.281 `repr(x)` works (was E0423/rejected), v0.1.280 float division by zero raises ZeroDivisionError, v0.1.279 `bool & | ^ bool` stays a bool, v0.1.278 `abs()` of an i64 overflow-checks (`abs(i64::MIN)` panics), v0.1.277 `sorted()` over a `list[float]` compiles (was E0277). (v0.1.13 PTX gate.) A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (str Unicode, bool-as-int, tie/stability, modpow, keyword identifiers, mut-receiver-in-condition, the i64-overflow contract trio left-shift/right-shift/abs, chained-compare double-eval, sorted-over-float, bool-bitwise type, float div-by-zero, repr(), float scientific notation).
