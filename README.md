...
- Latest tag: **`v0.1.206`** — **dataclass field defaults** (`timeout: int = 30` → omitted fields filled at construction; `Config()`, `Config(timeout=5)`). Recent: v0.1.205 dataclass keyword construction, v0.1.204 dataclass methods, v0.1.203 dataclass field assignment, v0.1.202 dataclass construction + field access (`Type::Struct`), v0.1.201 `@dataclass` → struct definition. (v0.1.13 PTX gate.)
