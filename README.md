...
- Latest tag: **`v0.1.508`** — a dataclass with a custom `__hash__` is now usable as a set element / dict key: it derived neither `Hash` nor `Eq` (and `__hash__` was dead code) so `set.add(pt)`/`d[pt]` failed (E0277/E0599); it now derives `Eq` + emits an `impl Hash` delegating to `__hash__` (a custom `__eq__` adds a hand `impl Eq`), mirroring the `__lt__`→`PartialOrd` dispatch. Recent: v0.1.507 a float with an align/width-only spec keeps Python's trailing `.0`; v0.1.506 a bare `range(...)` bound to a variable materializes to a `Vec<i64>`; v0.1.505 comprehension tuple loop vars no longer pollute function scope; v0.1.503 `except LookupError`/`ArithmeticError` catch only their member exceptions; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone — 240+ consecutive differential correctness fixes, HUNT-V14…V22, bringing transpiled Rust into exact agreement with CPython)**; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V22, ~1400 probes). v0.1.505 comprehension tuple loop vars no longer pollute function scope; v0.1.504 a bool needle in an int-set membership coerces to i64; v0.1.503 `except LookupError`/`ArithmeticError` catch only their member exceptions; v0.1.502 incompatible-type dict membership folds to `False`; v0.1.501 string-index OOB → catchable `IndexError`; v0.1.500 neg plain-width radix is sign-magnitude **(500th-patch milestone — 240+ consecutive differential correctness fixes, HUNT-V14…V21, bringing transpiled Rust into exact agreement with CPython)**; v0.1.499 `continue` in `range()` loops; v0.1.497 `d[k].pop()` mutates in place; v0.1.489 the typed `except` is an allowlist; v0.1.471 the dunder-dispatch cluster; v0.1.445 int↔float comparison exact above 2^53. **Full per-tag history: see [CHANGELOG.md](CHANGELOG.md).** A sustained run of correctness fixes comes from multi-pass adversarial differential python3-vs-rustc hunts (HUNT-V1…V21, ~1300 probes).
