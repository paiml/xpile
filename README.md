...
- Latest tag: **`v0.1.144`** — **generator expressions** (`sum(i*i for i in range(n))`). Recent: v0.1.143 context-aware subscript index + unary `-` (closes the ctx-free-position class), v0.1.142 context-aware collection literals, v0.1.141 context-aware comparison operands, v0.1.140 context-aware ternary branches, v0.1.139 `int(s, base)` radix parsing. (v0.1.13 PTX gate.)
