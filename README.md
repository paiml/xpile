...
- Latest tag: **`v0.1.89`** — Python **first-class closures** (`f = lambda y: y+1; f(x)`). Recent: v0.1.88 dict + set comprehensions over `range(...)`, v0.1.87 general slice step `xs[a:b:c]`, v0.1.86 in-place dict merge `a.update(b)`, v0.1.85 list comprehension over `range(...)`. (v0.1.13 PTX gate.)
