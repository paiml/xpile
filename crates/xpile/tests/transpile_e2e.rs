...
/// PMAT-1150: sets lower to indexmap::IndexSet (deterministic iteration), not
/// IndexSet (per-process RandomState → flapping). Order-independent ops MATCH.
#[test]
fn set_deterministic() {
    let rust = xpile_transpile_to_rust("set_deterministic.py");
    assert!(
        rust.contains("IndexSet")
            && !rust.contains("collections :: IndexSet")
            && !rust.contains("collections::IndexSet"),
        "sets use indexmap::IndexSet, not std IndexSet:\n{rust}"
    );
    let driver = r#"
fn main() {
