...
/-! ## PMAT-1183 — non-vacuous `render_idempotency` (skeptic follow-up).

    The original `render_idempotency` was `render contract config =
    render contract config` — reflexivity, TRUE FOR ANY `def`. Its
    docstring claimed to catch a ContractBackend that embeds a build
    timestamp in LaTeX, uses `rand` for autogenerated mdBook labels, or
    relies on HashMap iteration order in Lean's emitted attribute table,
    but a `= f x` reflexivity stub certifies nothing: a state-leaking
    backend satisfies `f x = f x` just as trivially as a deterministic
    one. (Same vacuity class as PMAT-1141/1176/1177/1178/1179/1180/1181/
    1182 — see `PROVABILITY-INVENTORY.md`.)

    The real determinism claim is STATE-INDEPENDENCE: `render`'s output
    depends only on `(contract, config)`, not on any ambient mutable
    state a non-pure impl might read (a build clock, a `rand` seed, the
    attribute-table hash order). We model that explicitly so the
    faithfulness of the ContractBackend is LOAD-BEARING and the theorem
    is FALSIFIABLE (the `≠` dual below exhibits a state-leaking backend
    that breaks it). This is the PROOF-LANE mirror of
    `XpileBackendTrait.lower_idempotency` (PMAT-1181) — and the FOURTH
    and FINAL primary trait-idempotency stub, completing the 2×2
    trait-determinism matrix (Frontend PMAT-1180 / Backend PMAT-1181 /
    ContractFrontend PMAT-1182 / ContractBackend this slice)
    non-vacuously on both lanes. -/

/-- Ambient mutable state a non-pure ContractBackend might read across
    render calls (a build timestamp folded into LaTeX, a `rand` seed for
    autogenerated mdBook labels, a HashMap iteration order for Lean's
    emitted attribute table). A FAITHFUL backend ignores it; a
    state-leaking one folds it into the emitted `RenderedDoc`. -/
structure ContractBackendState where
  bytes : Array UInt8
deriving DecidableEq

/-- A ContractBackend model carrying whether its `render` implementation
    leaks ambient `ContractBackendState` into the output. -/
structure StatefulContractBackend where
  leaks_state : Bool
deriving DecidableEq

/-- Stateful `render`. A faithful ContractBackend (`leaks_state = false`)
    emits `contract ++ config` regardless of the ambient state; a leaking
    one appends the state bytes (a `% rendered at …` timestamp, a random
    label), so its output varies with hidden state it should not
    observe. -/
def render_stateful (f : StatefulContractBackend)
    (st : ContractBackendState) (contract config : Array UInt8) : RenderedDoc :=
  if f.leaks_state then
    { bytes := contract ++ config ++ st.bytes }
  else
    { bytes := contract ++ config }

/-- The canonical faithful xpile ContractBackend — deterministic, ignores
    ambient state. -/
def xpileContractBackend : StatefulContractBackend := { leaks_state := false }

/-- A deliberately-broken ContractBackend that leaks ambient state — the
    witness that makes `render_idempotency` non-vacuous. -/
def leakyContractBackend : StatefulContractBackend := { leaks_state := true }

/--
  **Refinement theorem** for `render_idempotency` (the load-bearing
  claim from the contract YAML's equation block), restated
  NON-VACUOUSLY (PMAT-1183).

  Determinism as STATE-INDEPENDENCE: for the faithful xpile
  ContractBackend, `render` yields the same `RenderedDoc` for a fixed
  `(contract, config)` regardless of ambient mutable state `st₁`/`st₂`.
  This is `rfl` ONLY because `xpileContractBackend.leaks_state = false`
  — the faithfulness of `xpileContractBackend` is load-bearing (flip it
  to `true` and the theorem becomes FALSE, as
  `leaky_contract_backend_nondeterministic` witnesses).

  Falsification: a backend that includes `% rendered at
  2026-05-17T23:30:00Z` in LaTeX output, or a `rand` mdBook label,
  would falsify this theorem on consecutive calls — exactly
  `leakyContractBackend`. The Silver-tier refinement
  (`citation_round_trip_silver`) carries the typed citation claim; this
  Bronze theorem now carries a real determinism claim rather than a
  reflexivity tautology.

  Status: **discharged at v0.1.0 (PMAT-068), de-vacuoused (PMAT-1183)**.
  Tier: Bronze.

  This theorem completes the four-corner trait-determinism matrix
  NON-VACUOUSLY:
    - Frontend (PMAT-062 / de-vac PMAT-1180): code-lane parse determinism
    - Backend (PMAT-064 / de-vac PMAT-1181): code-lane emit determinism
    - ContractFrontend (PMAT-066 / de-vac PMAT-1182): proof-lane parse
    - ContractBackend (this theorem / PMAT-1183): proof-lane emit
-/
theorem render_idempotency
    (contract config : Array UInt8) (st₁ st₂ : ContractBackendState) :
    render_stateful xpileContractBackend st₁ contract config
      = render_stateful xpileContractBackend st₂ contract config := by
  rfl

/-- **`≠` DUAL** locking `render_idempotency` non-vacuous: a
    state-leaking ContractBackend is NON-deterministic — there exist two
    ambient states producing different `RenderedDoc`s for the same
    `(contract, config)`. If `render_idempotency` were a `= f x`
    reflexivity stub this would be UNPROVABLE. -/
theorem leaky_contract_backend_nondeterministic :
    ∃ (contract config : Array UInt8) (st₁ st₂ : ContractBackendState),
      render_stateful leakyContractBackend st₁ contract config
        ≠ render_stateful leakyContractBackend st₂ contract config := by
  refine ⟨#[], #[], { bytes := #[] }, { bytes := #[7] }, ?_⟩
  decide

/-- **Pin**: the faithful ContractBackend's output coincides with the
    pure `render` baseline for every ambient state — the positive
    companion to the divergence dual, keeping the original pure model
    load-bearing. -/
theorem faithful_contract_backend_matches_pure
    (st : ContractBackendState) (contract config : Array UInt8) :
    render_stateful xpileContractBackend st contract config
