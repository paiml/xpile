...
## PILOT — machine-checked (32 modules, in `lakefile.lean` roots)
...
| `PyContextManagerExit` | `C-PY-CONTEXT-MANAGER-EXIT` (PMAT-1131: NEW contract for user context managers PMAT-1072 (`with cm as x: BODY` → `x = __cm.__enter__(); try: BODY finally: __cm.__exit__()`). Core-only, import-free `WithLowering` two-phase (__enter__/__exit__) STRUCTURE EXTENSIONALITY, PLUS the FINALLY GUARANTEE — `exit_runs_always : ∀ o : Outcome, exitRuns o = true`: __exit__ runs on EVERY body outcome (ok | err), the reason the desugar uses a finally-only try (PMAT-1073) not a plain `enter; BODY; exit` sequence (which would skip __exit__ on a raise, leaking the resource) — with `exit_runs_on_ok`/`exit_runs_on_err` pinning both paths. provability/mathlib: a control-flow guarantee reduces to a finite case analysis over the two-constructor `Outcome`, closed by `decide` over CORE Lean 4 — NO Mathlib/sorry/axiom. Pilot 31 → 32; the FOUR major capabilities shipped this session (exceptions/generators/file-io/context-managers) all now under proven core-Lean contracts) |
