...
| 2026-07-31 | `0.1.618` | **pre-flight 1 never ran.** §1 prescribes a Wednesday `workflow_dispatch` of `release.yml` against the release-candidate SHA. `release.yml` has **two runs in its entire history and both are `push` events** (`ccb95a04` for `0.1.617`, `6b5f6c02` for `0.1.618`) — no dispatch run exists | not noticed at the time; nothing reads the workflow's run list. Pre-flights 2 (on-tag clean-room, SUCCESS) and 3 (local `--dry-run --locked`, exit 0) both ran, so what was lost is the redundancy, not the coverage |
| 2026-07-31 | `0.1.618` | **the §2a tripwire suite exited 101, and the batch proceeded anyway.** Armed all five blocking tripwires for the first time (prior pre-flights armed only `XPILE_REQUIRE_WASM_RUNTIME`): 333 suites, 3,071 passed, **2 failed** | both failures are in this repo's own witnesses, neither in the artefact. (a) `makefile_dialect_refusal_witness::make_and_the_shredded_shell_disagree_when_executed` probes with `tool_present("sh", "-c")` — `sh -c` with no operand is a usage error everywhere, so the probe fails *because* `sh` is present and correct, and the body has never executed on any host. (b) `release_plan_freshness_witness::no_plan_says_it_will_create_a_tag_that_already_exists` reds **because the tag exists**, which A1 also requires — so A1's two preconditions cannot both hold. `origin/main` is already corrected; the declaration survives only in the **tagged** tree, which §2a mandates running from and which is frozen, so this reds on every tag-pinned pre-flight for every release and no edit to `main` can reach it. A7 barred repairing either on the day; both land in `0.1.619` |
| 2026-07-31 | `0.1.618` | **partial batch, 30 of 31, recovered in ~90 seconds.** `cargo publish --workspace` exited 101 on the flagship `xpile` with `429 Too Many Requests` (crates.io rate-limits publishes to existing crates); the retry-after had already elapsed | **not an A4 stop.** A4 forbids retrying at the same version because a *succeeded* upload returns `409`. The registry was queried before anything was retried and `xpile 0.1.618` was **absent** (the sparse index still served `0.1.617`), so the upload had been rejected, not accepted. `cargo publish -p xpile` at the **same** version completed the intended batch. Nothing yanked, no version bumped, no crate routed around; all 31 verified live |
| — | — | the rows above are the **first written by §5 step 9 on the day**; every row before them is a retrospective reconstruction | — |
