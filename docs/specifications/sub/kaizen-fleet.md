...
xpile is **kernel tier** at v0.1.0. Phase 4 (Kani equivalence proofs on Layer-1/Layer-2 contracts) was the planned trigger for this transition; that phase shipped early and overshipped — every contract in the substrate (12 of 12) has a Kani BMC harness on every PR via the dedicated `kani` CI job (PMAT-021 / XPILE-QUORUM-003). The translation contracts produce verifiable kernels by construction: **165 Lean refinement theorems (58 Bronze + 97 Silver + 10 Gold)** in `contracts/lean/` + **43 Kani BMC harnesses** in `contracts/kani/` = **208 stratum-vote artifacts** (post-PMAT-127..138 the Lean Bronze side grew from 12 to 50; post-PMAT-147..151 / XPILE-QUORUM-006 the Kani side grew from 12 to 43; post-PMAT-156..162 the Silver bracket added 7 Silver theorems across single-equation contracts; post-PMAT-164..183 brought Silver to every equation in every contract — 42/42; post-PMAT-185..189 opened the Gold tier with 10 theorems across 5 contracts demonstrating refinement subtypes on Layers 1/2/4/5 using both bounded-numeric and collection-cardinality subtype patterns). See [phased-rollout.md](phased-rollout.md) "What actually shipped" for the comparison of planned vs. shipped phases.
