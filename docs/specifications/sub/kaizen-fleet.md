...
xpile is **kernel tier** at v0.1.0. Phase 4 (Kani equivalence proofs on Layer-1/Layer-2 contracts) was the planned trigger for this transition; that phase shipped early and overshipped — every contract in the substrate (12 of 12) has a Kani BMC harness on every PR via the dedicated `kani` CI job (PMAT-021 / XPILE-QUORUM-003). The translation contracts produce verifiable kernels by construction: **50 Lean refinement theorems** in `contracts/lean/` + **43 Kani BMC harnesses** in `contracts/kani/` = **93 stratum-vote artifacts** (post-PMAT-127..138 the Lean side grew from 12 to 50 as every equation gained its own Bronze-tier theorem; post-PMAT-147..151 / XPILE-QUORUM-006 the Kani side grew from 12 to 43 as every equation in the 5 multi-equation contracts gained its own symbolic harness). See [phased-rollout.md](phased-rollout.md) "What actually shipped" for the comparison of planned vs. shipped phases.
