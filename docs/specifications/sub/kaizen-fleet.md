...
xpile is **kernel tier** at v0.1.0. Phase 4 (Kani equivalence proofs on Layer-1/Layer-2 contracts) was the planned trigger for this transition; that phase shipped early and overshipped — every contract in the substrate (12 of 12) has a Kani BMC harness on every PR via the dedicated `kani` CI job (PMAT-021 / XPILE-QUORUM-003). The translation contracts produce verifiable kernels by construction: **76 Lean refinement theorems (50 Bronze + 26 Silver)** in `contracts/lean/` + **43 Kani BMC harnesses** in `contracts/kani/` = **119 stratum-vote artifacts** (post-PMAT-127..138 the Lean Bronze side grew from 12 to 50; post-PMAT-147..151 / XPILE-QUORUM-006 the Kani side grew from 12 to 43; post-PMAT-156..162 the Silver bracket added 7 Silver theorems across all single-equation contracts — the 2×2 trait matrix + FFI + PTX + bashrs; post-PMAT-164..169 added 19 more Silver theorems across all 6 multi-equation contracts, bringing Silver coverage to 12/12 of the substrate). See [phased-rollout.md](phased-rollout.md) "What actually shipped" for the comparison of planned vs. shipped phases.
