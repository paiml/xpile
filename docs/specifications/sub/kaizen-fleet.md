...
xpile is **kernel tier** at v0.1.0. Phase 4 (Kani equivalence proofs on Layer-1/Layer-2 contracts) was the planned trigger for this transition; that phase shipped early and overshipped — every contract in the substrate (12 of 12) has a Kani BMC harness on every PR via the dedicated `kani` CI job (PMAT-021 / XPILE-QUORUM-003). The translation contracts produce verifiable kernels by construction: **258 Lean refinement theorems (53 Bronze + 108 Silver + 24 Gold + 39 Platinum + 34 Diamond)** in `contracts/lean/` + **43 Kani BMC harnesses** in `contracts/kani/` = **301 stratum-vote artifacts** (Bronze + Silver + Gold + Platinum + Diamond depth-1 UNIVERSAL, **Diamond depth-2 UNIVERSAL across all 12 contracts (PMAT-228..239)**, **Diamond depth-3 UNIVERSAL across all 5 layers (PMAT-241..245)** — every layer has at least one contract with 3 distinct Diamond categories). See [phased-rollout.md](phased-rollout.md) "What actually shipped" for the comparison of planned vs. shipped phases.
