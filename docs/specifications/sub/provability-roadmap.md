...
**Status (XPILE-DIFF-002)**: ✅ shipped — PMAT-024. Generalises the runner from 1-arg-only to support 2-arg fixtures via per-arg input ranges (struct `FixtureCfg { args: &[(i64, i64)] }`); driver synthesis builds the right `entry(argv[0], argv[1], ...)` call expression at the arity. Three new 2-arg fixtures: `gcd(a, b)` over [0, 1M]², `multi_branch::range_size(a, b)` over [-1B, 1B]², `bits::bits(a, b)` over [-2^61, 2^61)². Total: **100 differential checks across 10 fixtures per CI run**, all green. Still pending under XPILE-DIFF: widen ranges to overflow-prone + interpret `.checked_*().expect(...)` panic as "Python promoted to BigInt" (XPILE-DIFF-003); add `square_plus` / `safe_div` and other tricky-semantics fixtures.
...
**Status (XPILE-REFINE-003)**: pending — the mul / floor_div / mod stub trio in `contracts/lean/PyIntArith.lean` still carries `by trivial` placeholders. They need different proof shapes than the additive case (likely `Int.bmod_mul_emod_self_left` and friends for multiplication; `Int.fdiv` / `Int.fmod` machinery for the floor-div pair). Tracked under `XPILE-PENDING-UNTIL: v0.3.0` so the deadline gate keeps the gap visible.
