...
**Status (PMAT-033)**: ✅ shipped — unified `xpile quorum` reporter consolidates all four strata into a single per-contract table. It's a reporter (not a gate); the constituent gates remain authoritative. Sources: Semantic = `lean_theorem:` refs in the contract's own YAML; Symbolic = `kani_harness:` refs in the contract's own YAML; Runtime = **the union of (a) fixture files mentioning the contract ID and (b) top-level `*.rs` files under each `--witness-dir` that BOTH mention the ID and carry a non-comment call to a `RUNTIME_PROBES` runtime-availability probe** (widened by PMAT-1367 — see the status entry below); Extrinsic = roadmap mentions (reuses PMAT-032 scanner). Quorum thresholds per §14.4: ≥1 vote in ≥3 strata = QUORUM, 1-2 strata = PARTIAL, 0 = UNVERIFIED. At the time this entry was written the substrate was 12 contracts and all 12 reached QUORUM at the 4-stratum minimum; **the live figures are DERIVED, never typed here — run `xpile quorum` (or `xpile quorum --json`) for the current per-contract table and totals.** Tests: `tests/quorum.rs` asserts C-PY-INT-ARITH has full quorum and the reporter walks every contracts/*.yaml without missing any. Still pending under XPILE-QUORUM: §14.5 F3 pairwise-correlation guard (XPILE-QUORUM-004 — needs ≥3 oracles per contract; C-PY-INT-ARITH qualifies today with 4 distinct stratum sources).

**Status (PMAT-1367)**: ✅ shipped — the **Runtime stratum counts executions, not mentions**. `count_runtime_witnesses` was a flat `read_dir` over `crates/xpile/tests/fixtures` whose entire body was `if text.contains(contract_id) { hits += 1 }`. A bare comment in a fixture scored a full Runtime vote, while the WASM witness corpus under `crates/xpile-wasm-codegen/tests` — which assembles emitted modules with `wat2wasm` and executes them under `wasm-interp`, inside the REQUIRED `workspace-test` job, with `XPILE_REQUIRE_WASM_RUNTIME=1` turning a missing runtime into a hard panic — scored **zero**, leaving `C-COMPILE-RUST-TO-WASM` and `C-WASM-HEAP` at `Run=0` / PARTIAL while being the two most-executed contracts in the repo.

The rule is a WIDEN-ONLY union: pass A (fixtures) is kept byte-identical, so no existing vote can disappear; pass B adds top-level `*.rs` under each repeatable `--witness-dir` that BOTH names the ID and carries a non-comment call to one of `RUNTIME_PROBES` (today exactly `wasm_runtime_available(` — the same constant `witness_floor.rs` floors the executing half of the corpus on). Votes union into a `BTreeSet` of canonical paths, so overlapping `--witness-dir` arguments cannot double-count. Default `--witness-dir` is `crates/xpile-wasm-codegen/tests` only.

**Why the conjunction, measured rather than assumed:** a naive "any `crates/*/tests/*.rs` naming the ID" rule over the live 193-file set flips **ten of the eleven** PARTIAL rows and **eight are unearned** — six from gate files (`contract_citation_integrity.rs` hardcodes a roster of IDs, `lean_pilot_roots.rs` names them in comments; every `Command::new` in them spawns the `xpile` binary itself, so `Command::new` is not a proxy for execution) and two from `gpu_witness.rs`, which carries a real probe but takes the `NotRun { no-engine }` branch on every CI runner — a vote resting on evidence the required job has never produced. `tests/quorum.rs` therefore pins six named contracts at `runtime == 0` / PARTIAL as MANDATORY negative assertions, alongside a `runtime >= 50` FLOOR (not `>= 1`) on the two WASM rows so corpus deletion is visible, plus unit tests in `mod quorum_tests` proving that naming an ID without a probe scores 0, that a probe named only inside a `//` comment scores 0, and that a file reachable from both passes counts once.
