...
- **v0.1.0 (current)**: no bashrs dependency yet. The merge plan above is the design target; only the depyler / decy / ruchy workspace crates exist as scaffolds.
- **v0.2.0 (target — Q3 2026)**: Layer A complete (bashrs source folded into the workspace via `git filter-repo` + `git subtree add`). bashrs-frontend / bashrs-backend scaffold-stage crates exist; the bashrs binary becomes xpile-internal; `paiml/bashrs` repo is a re-export shim. Meta-HIR shell variants land but only `bashrs-frontend` produces them and only `bashrs-backend` consumes them. Other backends return `Unsupported`.
