...
3. A Lean theorem in `contracts/lean/` that depends on a meta-HIR shell variant (e.g., proving `subprocess invariants` for transpiled Python). **Partly discharged (PMAT-469, 2026-09-25):** `Shell.composition_idempotence_diamond` in `contracts/lean/Bashrs.lean` proves `run c (run c s) = run c s` for `mkdir -p`, `touch`, `chmod`, `>` and `VAR=x`, and proves that `>>` and the sequence chmod-then-touch are NOT idempotent. The theorem quantifies over a Lean model of those commands, not over the meta-HIR `Stmt::Cmd` itself. `crates/xpile/tests/bashrs_idempotence_seam_witness.rs` binds the model to the shipped path: 10 pins are transpiled through bashrs-frontend → meta-HIR → bashrs-backend, run once and twice, and each result must match the model's observation. An emitter that doubled each command turns the append pins red.
