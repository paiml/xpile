...
- **v0.1.0 (current — Layer A scaffold + first Layer B variant shipped 2026-05-17 across PRs #68/#69/#PMAT-039)**: `crates/bashrs-frontend/` and `crates/bashrs-backend/` exist as workspace members alongside `depyler / decy / ruchy` siblings. `SourceLang::Shell` + `Target::Shell` enum variants land (PMAT-037). `Frontend::matches_path` enables canonical-filename routing for `Makefile` / `Dockerfile` (PMAT-038). **First Layer B meta-HIR shell variant shipped (PMAT-039): `Stmt::Cmd { program, args }`** — bashrs-frontend parses real shell input via a line-based parser, bashrs-backend emits real (if minimal) POSIX sh. Other backends refuse `Stmt::Cmd` via explicit `Unsupported` arms naming `C-BASHRS-POSIX-IDEMPOTENCE` — the load-bearing cross-domain dispatch boundary. New `contracts/bashrs-posix-idempotence-v1.yaml` (`C-BASHRS-POSIX-IDEMPOTENCE`, kind: pattern) registers the bashrs domain in the contract substrate; `xpile quorum` walks 12 contracts. **No real bashrs source folded yet** — the full ShellIR + 17,882-pattern corpus + ShellCheck-compatible verifier come at v0.2.0.
