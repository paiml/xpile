...
|----|------------------|--------|----------|
| F1 | **Believed-required check unenforced**: ruleset requires only `gate`; `workspace-test` — the job that executes every in-tree differential witness — is required-by-comment only (ci.yml:50) | **FIRED** | ruleset JSON vs ci.yml |
