...
*   **Citation Bridge Fragility (Mitigated but Present):** The prior reliance on regex for mapping Lean theorems back to YAML contracts has been structurally reinforced: it now utilizes structured citation constructs (`@[xpile_contract]`) parsed directly by Lean's native elaborator. While a massive improvement, manual refactoring or renaming in the proof lane can still silently break `citation_round_trip` invariants if not mediated by structured IDE tooling.
*   **Fixture Overfitting (Roadmap Mitigated):** The baseline Oracle validation is only as strong as its fixtures. However, the newly adopted **N-of-M Stratified Oracle Quorum** and **Differential execution checks** (running interpreter vs transpiled-binary on probar-generated inputs) are specifically designed to close this gap. Until `XPILE-QUORUM-001` and `XPILE-DIFF-001` (per [§27](../../docs/specifications/sub/provability-roadmap.md)) ship, the standard Oracle remains vulnerable to edge cases (e.g., negative zero, FFI pointer aliasing, unaligned memory).
