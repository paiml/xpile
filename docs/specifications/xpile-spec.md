...
- 🟢 Types beyond int/bool — **v0.2.0 Track 1.A string lane SHIPPED + substrate fully ratcheted** (PMAT-449/450/451/452/453/454): `Type::Str` + `Expr::LitStr` + `Expr::Concat` + f-string lowering all end-to-end through depyler-frontend → Rust/Ruchy/Lean backends; contract `C-XLATE-PY-STR-TO-RUST-STRING` at QUORUM, **depth-13 UNIVERSAL** (all 13 diamonds wired across Templates 1, 2, 6, 6b, 6c, 9, 10, 11, 12, 13). **The Track 1.A exit criterion `def greet(name: str) -> str: return f"Hello, {name}!"` transpiles cleanly to all three backends, rustc round-trip green.** Substrate now: **13 contracts at QUORUM**, **184 Diamond theorems**, depth-13 UNIVERSAL **strict equality CI gate restored** (was transitionally relaxed in PMAT-450, retightened in PMAT-454). `list[T]`, `dict[str,T]`, `float` are subsequent sub-tracks per [`sub/v0.2.0-depyler-merger.md`](sub/v0.2.0-depyler-merger.md).
