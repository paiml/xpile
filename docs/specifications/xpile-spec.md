...
- ⏳ **PMAT-26X+** — light up `rustc_codegen_nvvm` path in xpile-ptx-codegen (replaces `ScaffoldPtxEmitter` in the `general` slot). **Candidate third general emitter:** NVlabs `cuda-oxide` (pure-Rust MIR→Pliron→LLVM→PTX) — categorically independent of NVVM-IR, so it strengthens the §14.10 anti-correlation guard. Tracked-not-scheduled (LLVM 21 + sm_90/Hopper-vs-sm_89/Ada + alpha churn); see §30 "Watched bets."
...
**This subsection is the canonical pickup order for autonomous sessions.** A session **picks the highest-ranked open item below and executes it to a tagged release without asking**, per [`/CLAUDE.md`](../../CLAUDE.md). Ranking is by **expected value = (capability or epistemic gain) ÷ effort**, biased toward items that (a) unblock the most real-world code, (b) are verifiable by `rustc` round-trip (the proven method), and (c) are **not** the frozen depth-13 Diamond treadmill.
...
**Execution posture:** ship each rank as its own PR (full CI gate green) → squash-merge → tag a release, then take the next. **R1–R5 are the autonomous-loop core** (low-risk, `rustc`-verifiable, ~one release each). R6–R10 are larger and sequenced after. **Always commit before launching an in-tree review agent** — agents have reverted uncommitted work (see [`/home/noah/.claude/.../feedback-commit-before-tree-agents.md`]); re-run the full gate after any review pass.
...
- **`cuda-oxide` (NVlabs) — pure-Rust→PTX compiler.** [github.com/NVlabs/cuda-oxide](https://github.com/NVlabs/cuda-oxide). NVIDIA-Research-affiliated, experimental/alpha (first release ~2026-05-07, `v0.2.1` 2026-06-10). Compiles single-source SIMT Rust kernels (`#[kernel]`) directly to PTX via a *fully Rust-native* pipeline — Rust → Stable MIR (`rustc_public`) → Pliron IR (an MLIR-like framework written in Rust, no C++/CMake/tablegen) → LLVM → PTX — so the whole compiler builds with `cargo`.
  ...
  - **Why it is NOT scheduled (blocked on all three of toolchain + hardware + maturity):** (1) **LLVM 21+** hard dependency — adopting it forces an ecosystem-wide LLVM bump xpile has not taken. (2) **Hardware mismatch** — its flagship device abstractions (TMA, thread-block clusters) are **Hopper-class (sm_90)**; the available hardware is **Ada (sm_89)**, so the quorum's Runtime stratum (`DiffExec` on a real GPU — the entire point of §29) cannot execute-verify those kernels here. (3) **Alpha churn** — the README warns "expect bugs, incomplete features, and API breakage" (e.g. documented `index_2d(stride)` unsoundness); building on it now imports that instability. (4) The xpile PTX lane it would plug into is *itself* still scaffolded (§29 `PMAT-26X+` is ⏳) — a third emitter cannot precede its own consumer.
  - **Disposition:** **tracked, not scheduled.** Zero near-term EV — orthogonal to the R-ladder (all `rustc`-round-trip-verifiable CPU capability) and gated on developments outside the repo. **Re-evaluate** when (a) LLVM 21 is in xpile's pinned toolchain, (b) Ada/Blackwell support lands or sm_90 hardware is available for the Runtime stratum, and (c) `cuda-oxide` exits alpha — or at the next SOTA dossier (2026-11-15), whichever is first. It is *not* an R-item and must not be auto-picked-up.
