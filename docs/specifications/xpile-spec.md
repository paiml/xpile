...
| **PMAT-609** — `list.pop(i)` runtime negative index (correctness) | `xs.pop(i)` (i<0 at runtime) | ✅ SHIPPED v0.1.308 — **fixed panic-mismatch**: a runtime negative pop index emitted `(xs).remove((i) as usize)` → `usize::MAX` → panic, where Python `pop(i<0)` removes from the end (`i+len`). Literal `pop(-1)` already resolved to `len - k`; only the runtime case broke. FIX (frontend): normalize a non-literal index (bind once, `if __pidx < 0 { len + __pidx } else { __pidx }`); extended codegen `expr_mentions_ident` with IfExpr/Block arms so it binds before the mutable `remove`. Literal pops unchanged. No new IR. Hunt #5 (H5-24). Found by the differential hunt |
| **PMAT-610** — `int(s)` PEP 515 underscores (correctness) | `int("1_000")` | ✅ SHIPPED v0.1.309 — **fixed panic-on-valid**: `int(str)` emitted `(s).trim().parse::<i64>()`, but Rust's parser rejects the PEP 515 underscore separators Python accepts (`int("1_000")`==1000) → panic. FIX (rust+ruchy codegen): validate Python's between-digits rule (no leading/trailing/doubled underscore on the post-sign body — exact for an int) then strip + parse; invalid placements (`_1`/`1_`/`1__0`) still raise ≈ ValueError. `float(s)` + `int(float)` unchanged (float(s) underscore is a follow-up). No new IR. Hunt #5 (H5-12). Found by the differential hunt |
| **PMAT-611** — `float(s)` PEP 515 underscores + E0716 temp fix (correctness) | `float("1_000.5")`, `float("inf")` | ✅ SHIPPED v0.1.310 — **fixed panic-on-valid** (float twin of PMAT-610): `float(str)` emitted `(s).trim().parse::<f64>()` → rejects PEP 515 underscores Python accepts (`float("1_000.5")`==1000.5, `float("1.5e1_0")`==1.5e10). FIX (rust+ruchy codegen): validate Python's exact rule — every `_` has an ASCII digit on both sides (covers the fractional/exponent parts) — then strip + parse; invalid placements (`1_.5`/`1.5_`/`_1.0`/`1_e5`) still raise ≈ ValueError. **Also fixed a latent E0716** in both the new float block and the shipped PMAT-610 int block: binding the trimmed `&str` dropped a temporary-`String` operand (`float("inf")`, `int("1_000")`) while borrowed (caught by `math_round_overflow` in CI) → both now bind a *reference* (temporary lifetime extension; a reused variable operand is not moved). No new IR. Hunt #5 (float twin of H5-12). Found by the differential hunt |
| **PMAT-612** — `round(int, n)` → int (correctness) | `round(12345, -2)`, `round(x, 2)` | ✅ SHIPPED v0.1.311 — **fixed E0425**: `round(int, n)` emitted a bare `round(x, n)` call (undefined fn) → transpile succeeds, invalid Rust; even `round(count, 2)` failed to compile. FIX (new `Expr::RoundIntToDigits`, rust+ruchy codegen): `n >= 0` is the identity (a non-negative literal folds to `return value`); `n < 0` rounds to the nearest `10^(-n)` with round-half-to-**even** (banker's: `round(12350,-2)`==12400, `round(12250,-2)`==12200). Arithmetic in `i128` (scale + products can't overflow), result fails loud (C-PY-INT-ARITH) out of i64 range. Lean refuses. Hunt #4 (H4-2). Found by the differential hunt |
| **PMAT-613** — f-string radix of a negative int → sign-magnitude (correctness) | `f"{-255:x}"` | ✅ SHIPPED v0.1.312 — **fixed silent two's-complement**: a bare f-string radix spec (`:x`/`:X`/`:b`/`:o`) emitted `format!("{:x}", n)` = Rust two's-complement (`ffffffffffffff01`), but Python is sign-magnitude (`f"{-255:x}"`=="-ff"). FIX (frontend only, NO new IR): reuse `Expr::IntRadixStr` (`prefixed: false`) — emits `sign + format(unsigned_abs)`, matching Python + the `hex`/`bin`/`oct` builtins. Radix-with-width keeps FormatSpec (negative+width padding deferred); `format(x, "x")` builtin fixed too (shared path). Hunt #4 (H4-3). Found by the differential hunt |
| **PMAT-614** — float `a // b` is CPython `float_divmod` (correctness) | `1.0 // 0.1`, `inf // 2` | ✅ SHIPPED v0.1.313 — **fixed silent over-rounding**: float `//` emitted `(a/b).floor()`, but Python is CPython `float_divmod` — the naive floor over-rounds the textbook `1.0 // 0.1` to 10.0 (Python 9.0; also 2.0//0.1→19, 5.5//1.1→4) and mishandles infinite operands (`inf // 2`==nan, `-5.0 // inf`==-1.0). FIX (rust+ruchy codegen, NO new IR): replicate CPython — `mod = fmod(a,b)` (Rust `%`), `div = (a-mod)/b`, `div -= 1` when remainder/divisor signs differ, then `floor` with the `div-floor > 0.5` round-up correction. Evaluate-once temps; ZeroDivisionError guard preserved. Verified vs python3 across 20 cases. Hunt #5 (H5-23, broadened from inf to the common finite case). Found by the differential hunt |
| **PMAT-615** — augmented set ops compile (correctness) | `s -= other`, `s \|= other` | ✅ SHIPPED v0.1.314 — **fixed transpile→invalid-Rust**: `s -= other` emitted `HashSet::checked_sub` (E0599) and `s \|= / &= / ^= other` emitted owned-value `\|`/`&`/`^` on `HashSet` (E0369); the non-augmented forms already worked. FIX (frontend `combine_aug`, NO new IR): when both operands are sets, route the augmented op to the binop `SetOp` path (difference/union/intersection/symmetric_difference), like the dict-`\|=`/list-`+=` special-casing. Cross-checked vs python3. Hunt #6 (re-verified on a fresh binary after the hunt ran against a stale v0.1.288 artifact). Found by the differential hunt |
| **PMAT-616** — sorting a float list with NaN no longer panics (correctness) | `sorted([nan, 1.0, 2.0])` | ✅ SHIPPED v0.1.315 — **fixed panic-on-valid**: float sorts emitted `partial_cmp(...).unwrap()`, which panics on NaN, but Python's `sorted`/`list.sort` does not raise on NaN (undefined-but-non-crashing order). FIX (rust+ruchy codegen, NO new IR): all 6 float-sort comparator sites (keyless asc/desc, in-place sort/sort-reverse, float key= asc/desc) use `partial_cmp(...).unwrap_or(Equal)` — byte-identical for finite floats (finite sorts stay python3-exact), no crash on NaN. Python's NaN order is unreplicable (invalid comparator), so the NaN order is unspecified-but-defined. Hunt #6 (re-verified on a fresh binary). Found by the differential hunt |
| **PMAT-617** — `bool` compared with `int` coerces (correctness) | `flag == 1`, `True < 2` | ✅ SHIPPED v0.1.316 — **fixed transpile→invalid-Rust**: a bool/int comparison emitted a bare `bool OP i64` → E0308 (Python's bool is an int subtype, `True == 1`). FIX (frontend `build_chain_cmp`, NO new IR): coerce the bool side to `i64`, using the authoritative operand types (not re-inference, so a chained `__cmpN` temp is also coerced). The comparison half of the bool-as-int story (PMAT-565 did arithmetic). Covers simple + chained; bool/bool needs no coercion. Hunt #7 (fresh binary). Found by the differential hunt |
| **PMAT-618** — `d.get(k) == v` / `!= v` compiles (correctness) | `d.get(k) == 5` | ✅ SHIPPED v0.1.317 — **fixed transpile→invalid-Rust**: a no-default `d.get(k)` is `Option<T>`, so `d.get(k) == 5` emitted `Option<i64> == i64` → E0308. Python returns `None` when absent (`None == 5` is `False`), modeled exactly as `Option<T> == Some(5)`. FIX (rust+ruchy codegen, NO new IR): for `==`/`!=` where exactly one operand is a `DictGetOpt`, wrap the bare-value side in `Some(...)`. Scoped to `==`/`!=` (a `<`/`>` on a possibly-`None` is a Python `TypeError`). Hunt #7 (H7-7, fresh binary). Found by the differential hunt |
| **PMAT-619** — 3-arg `pow` neg base + neg modulus (correctness) | `pow(-2, 3, -5)` | ✅ SHIPPED v0.1.318 — **fixed silent wrong value**: PMAT-605 re-signed the modpow result for a negative modulus, but the base normalization + `% m` reductions still assumed `m > 0`, so a negative base with a negative modulus gave a wrong value (`pow(-2,3,-5)` → 3, Python -3). FIX (rust+ruchy codegen, NO new IR): run the whole modexp on the magnitude `|m|` (i128), then sign-correct to the modulus's sign. Verified vs python3 across 17 cases. Completes the 3-arg `pow` story (571/604/605/607/619). Hunt #7 (H7-18). Found by the differential hunt |
| **PMAT-620** — reject no-default `d.get(k)` in an f-string (correctness) | `f"{d.get(k)}"` | ✅ SHIPPED v0.1.319 — **fixed transpile→invalid-Rust**: a no-default `d.get(k)` is `Option<T>` (no `Display`), so `f"{d.get(k)}"` emitted `format!("{}", Option)` → E0308. `str()`/`print()` of a bare Optional already reject; the f-string field was the lone leak. FIX (frontend f-string field lowering, NO new IR): reject a `DictGetOpt` field with a clear message (use `d.get(k, default)` / `d[k]`) — fail-loud, consistent. Supported forms (`d.get(k, default)`, `d[k]`) unchanged. Rendering a bare Optional → deferred Optional sub-track. Hunt #7 (H7-9). Found by the differential hunt |
| **PMAT-621** — `str.split(sep, neg maxsplit)` splits unlimited (correctness) | `"a,b,c,d".split(",", -1)` | ✅ SHIPPED v0.1.320 — **fixed silent wrong output**: Python's negative maxsplit means "no limit", but xpile's `splitn((maxsplit as usize) + 1, sep)` wrapped (`usize::MAX + 1 == 0` → `splitn(0)` → zero parts). FIX (rust+ruchy codegen, NO new IR): `.saturating_add(1)` instead of `+ 1` — a negative maxsplit saturates to `usize::MAX` → all parts. Positive maxsplit unchanged. Verified vs python3. Hunt #8 (H8-7). Found by the differential hunt |
| **PMAT-622** — sort a list whose element embeds a float (correctness) | `sorted(list[tuple[float, int]])` | ✅ SHIPPED v0.1.321 — **fixed transpile→invalid-Rust**: a tuple-with-float / nested-float-list element emitted `Vec::sort()` (needs `Ord`; f64 not Ord) → E0277. FIX (frontend, NO new IR/codegen): recursive `type_contains_float` (F64 | tuple-with-float | nested-list-with-float) at the three keyless-sort detection sites routes to the existing `partial_cmp` path (PMAT-578/616). Int-only tuple keeps `.sort()`. Verified vs python3. Hunt #8 (H8-13). Found by the differential hunt |
| **PMAT-623** — interpolate a list in an f-string (correctness) | `f"{xs}"` | ✅ SHIPPED v0.1.322 — **fixed transpile→invalid-Rust** (COMMON): `f"{xs}"` over a list emitted `format!("{}", Vec)` → E0277 (Vec no Display). FIX (frontend, NO new IR): desugar to `"[" + ", ".join([repr(e) for e in xs]) + "]"` reusing Map + str.join + Concat + per-element repr (ToStr / True-False / quoted ReprStr); recursive for nested lists. Correct for every element type (where Rust `{:?}` would diverge — `[true]`/`['a']`). Verified vs python3. Hunt #8 (H8-2). Found by the differential hunt |
| **PMAT-624** — interpolate a tuple in an f-string (correctness) | `f"{p}"` | ✅ SHIPPED v0.1.323 — **fixed transpile→invalid-Rust**: `f"{p}"` over a tuple emitted `format!("{}", tuple)` → E0277 (no Display). FIX (frontend, NO new IR): heterogeneous per-position desugar `"(" + repr(p.0) + ", " + … + ")"`, tuple bound once in a `Block`, each position via `pyrepr_of` (Tuple arm → recursion). Single-element `(x,)` comma handled. 1-tuples blocked by a separate `(T)`-vs-`(T,)` type-emission bug (fixed in PMAT-625). Verified vs python3. Hunt #8 (H8-12). Found by the differential hunt |
| **PMAT-625** — emit a 1-element tuple as `(T,)`/`(x,)` (correctness) | `tuple[int]` param + `p[0]`, `(x,)` | ✅ SHIPPED v0.1.324 — **fixed transpile→invalid-Rust**: a 1-element tuple type/value emitted `(T)`/`(x)` (parenthesized value, not a tuple) → `.0`/indexing E0610. FIX (rust+ruchy codegen, NO new IR): both the `Type::Tuple` and `TupleLit` emitters add a trailing comma when `len == 1`; multi-element unaffected. Also un-blocks 1-element tuple f-string repr (PMAT-624). Verified vs python3. Found while shipping PMAT-624 |
| **PMAT-626** — `str()`/`print()` of a list or tuple (correctness) | `str([1,2,3])`, `print((1,2))` | ✅ SHIPPED v0.1.325 — **fixed reject**: `str(list)` fell through (→ I64 mismatch) and `print(list)` was declined. FIX (frontend, NO new IR): str() dispatch + print() arg-loop gain `List`/`Tuple` arms reusing `build_list_repr`/`build_tuple_repr` (PMAT-623/624) → the Python repr; nested via recursive `pyrepr_of`. Completes the list/tuple repr surface (f-string + str + print). Verified vs python3. Hunt #8 (H8-3). Found by the differential hunt |
| **PMAT-627** — fill defaults in nested user-calls in arg position (correctness) | `f(g(x))` (g has a default) | ✅ SHIPPED v0.1.326 — **fixed transpile→invalid-Rust**: a default-using fn called in argument position left the inner call under-applied → E0061 (outer reorder fills defaults but the delegated context-free `lower_call` lowers args via `lower_expr`, skipping reorder for nested calls). FIX (frontend, NO new IR): new `reorder_nested_call_args` recurses through Call-shaped args, default-filling each (and deeper). A default-call in a non-call container stays a loud E0061. Verified vs python3. Hunt #8 (H8-16). Found by the differential hunt |
| **PMAT-628** — clone a reused non-Copy var in a list literal / append (correctness) | `[inner, inner]`, `g.append(row)` ×2 | ✅ SHIPPED v0.1.327 — **fixed transpile→invalid-Rust**: a non-Copy var used >1× in a list literal or appended twice moved-then-used → E0382. FIX (frontend, NO new IR): shared `clone_if_reused_non_copy` (Ident + read_counts>1 + non-Copy → Clone) applied at the list-literal element + append/add element lowering (mirrors PMAT-588). Distinct/Copy untouched. Python aliases the object; clone gives copies (documented PMAT-569 divergence) but now compiles. Verified vs python3. Hunt #8 (H8-0/1). Found by the differential hunt |
| **PMAT-629** — `s *= n` / `xs *= n` are repetition (correctness) | `s *= 3`, `xs *= 3` | ✅ SHIPPED v0.1.328 — **fixed transpile→invalid-Rust**: `s *= n` emitted `String::checked_mul` (E0599); `xs *= n` was rejected. Both are repetition. FIX (frontend, NO new IR): `combine_aug` routes a `Mult` with a str/list operand through `try_repeat` → `Expr::Repeat`; the list aug-assign branch handles `*=`. int `*=` stays numeric. Verified vs python3. Hunt #9 (H9-11). Found by the differential hunt |
| **PMAT-630** — lower user-call args context-aware (correctness) | `g(5, c and d)` | ✅ SHIPPED v0.1.329 — **fixed reject**: a bool `and`/`or`/`not`/ternary arg to a user call was rejected ("operands must be Bool") because `lower_call` lowers args context-free (losing param types). FIX (frontend, NO new IR): a Name-callee user call lowers its args via `lower_expr_in_ctx` (context-aware) + builds `Expr::Call`; `len`/non-Name fall back. The proper general fix for the `lower_call`-is-context-free root (cf. PMAT-627). Full workspace test clean. Verified vs python3. Hunt #9 (H9-15). Found by the differential hunt |
| **PMAT-631** — `raise E(msg)` typed panic payload (correctness) | `raise ValueError("x")` | ✅ SHIPPED v0.1.330 — **fixed type-dropping**: `raise E(msg)` emitted `panic!("{}", msg)`, dropping the type, so distinct exception types were indistinguishable. FIX (frontend `lower_raise_stmt`, NO new IR): payload becomes `xpile: <Type>: <msg>` (matching the builtin convention). Identifies the type in the crash; groundwork for typed `except` matching. **Typed-exceptions epic, sub-slice 1 of 2** — sub-slice 2 (`except E` payload-match + re-raise, needs a TryCatch IR field + typing native panics OOB→IndexError) is deferred. Hunt #9 (H9-5..10). Found by the differential hunt |
| **PMAT-632** — `str.rjust`/`ljust`/`center` optional fill char | `"ab".rjust(5, "*")` | ✅ SHIPPED v0.1.331 — **fixed reject**: the 2-arg form (custom pad char) was refused (str-method arity fixed at 1). FIX (frontend + both backends, NO new IR): `ljust`/`rjust`/`center` accept `arity` OR `arity+1` args; with a fill char the codegen pads manually by repeating the fill string to the deficit char count (`format!` width can't take a dynamic fill); `center` keeps the CPython left-bias; the 1-arg space-fill path is unchanged. Verified vs python3 (fill / odd-width bias / already-wide no-op / sign-zero). Found by a differential probe |
| **PMAT-633** — stepped string slices `s[a:b:step]` | `"abcdef"[::2]`, `s[::-2]` | ✅ SHIPPED v0.1.332 — **fixed reject**: list slicing handled an integer step but str slicing refused it. FIX (frontend + both backends, NO new IR — `Expr::Slice` already has `step`+`of_str`): a str slice is already char-indexed (`Vec<char>`, PMAT-567) so the step machinery applies; frontend drops the str rejection (positive step, any bounds; negative step under the same unbounded constraint as list), and the codegen step arms collect str into `String` vs list into `Vec`. `s[::-1]` reverse unchanged. Full str/list step parity. Verified vs python3 incl Unicode (`"αβγδεζ"[::2]`→`"αγε"`). Found by a differential probe |
| **PMAT-634** — for-range loop-variable semantics (correctness) | `for i: for i:`, post-loop `i`, empty range | ✅ SHIPPED v0.1.333 — **fixed silent miscompiles**: `for i in range(...)` used the USER variable as the while-counter, so (1) nested same-name loops clobbered the outer counter (ran once), (2) post-loop `i` was `stop` not the last value Python leaks, (3) an empty range clobbered a pre-existing variable to `start`. FIX (frontend `lower_for_stmt` only, NO new IR): a fresh `LoweringCtx::fresh_loop_counter()` mints a per-loop synthetic counter `__forc{N}`; the user var is assigned from it inside the body (only when the body runs). A fresh var still leaks like Python; a pre-bound var survives an empty range. All 3 verified vs python3; other range forms unaffected. For-loop-variable cluster (H9-0/2/17). Found by a differential probe |
| **PMAT-635** — range-comprehension variables don't leak (correctness) | `def f(i): [i for i in range(3)]; return i` | ✅ SHIPPED v0.1.334 — **fixed silent miscompile**: a range comprehension (list/dict/set) desugared to a function-scope counter loop, so the comp variable leaked into the enclosing scope and clobbered a same-named outer binding (returned 3 not 99); Python comp variables are scoped to the comprehension. FIX (frontend only, NO new IR): generalize the `_`-rename (`active_rename: Option<(String,String)>`) + new `enter_comp_var` renames every range-comp counter (named or `_`) to a fresh `__forc{N}`; body reads resolve via the rename, the original name is never bound → no leak. Same class as PMAT-634, separate code path. List-iterable comps + genexprs already correct. Verified vs python3. Found by a differential probe |
| **PMAT-636** — `int ** <neg int literal>` → float | `2 ** -1` | ✅ SHIPPED v0.1.335 — **fixed reject**: an int raised to a negative power is a float in Python (`2 ** -1 == 0.5`), but xpile lowered `int ** int` to the integer `checked_pow` path unconditionally (can't represent a negative exponent → lower error). FIX (frontend only, NO new IR / no inference change): both `**` arms detect a negative integer-literal exponent from the AST (`extract_step_literal`, before the operands move) and route it through the existing float-power path (`FloatBinOp::Pow` → `powf`, → F64). A variable/non-literal negative exponent (runtime-dependent type) stays on the integer path. Verified vs python3; non-negative int powers + float powers unaffected. Found by a differential probe |
| **PMAT-637** — `x or default` / `x and y` return the operand | `x or 5`, `s or "d"` | ✅ SHIPPED v0.1.336 — **fixed reject**: Python `or`/`and` return the OPERAND by truthiness (`0 or 5 == 5`), not a Bool; xpile required Bool operands and rejected the default-value idiom. FIX (frontend `lower_bool_op_in_ctx`, NO new IR): for the common form (2 operands, first an `Ident`, same non-bool type — int/str/list/dict/set), lower to `IfExpr { cond: <a truthy>, then/else: operand }` (a is borrowed in the cond, cloned in the value branch → never moved). Bool operands keep `||`/`&&`; chains / non-Ident first / mixed types fall through (unchanged). Verified vs python3; boolean logic unaffected. Found by a differential probe |
| **PMAT-638** — short-circuit chains `a or b or c` | `name or env or "default"` | ✅ SHIPPED v0.1.337 — extends PMAT-637 to chains `a or b or c` / `a and b and c` (the multi-fallback idiom). FIX (frontend `lower_bool_op_in_ctx`, NO new IR): generalize the 2-operand case to an N-operand right-to-left `IfExpr` fold; leading operands must be `Ident`s (borrowed-in-cond / cloned-in-value), the last may be any expression, all the same non-bool type. N=2 reduces to the PMAT-637 form. Bool operands keep `||`/`&&`. Verified vs python3; 2-operand + boolean logic unaffected. Found by a differential probe |
| **PMAT-639** — runtime-negative list index wraps (correctness) | `xs[i]` with i<0 at runtime | ✅ SHIPPED v0.1.338 — **fixed silent panic**: `xs[i]` emitted `xs[i as usize]`, so a runtime-negative `i` (`-1`) underflowed to `usize::MAX` and panicked, where Python wraps (`xs[-1]` = last). FIX (both backends' `Expr::Index` arm, NO new IR): mirror the existing str-index negative-wrap — a non-negative literal index keeps the bare fast path; any other index binds the collection (by ref, eval-once) + index, then `if __li < 0 { len + __li } else { __li }`; out-of-range still panics (≈ IndexError). `Expr::Index` is list-only. Verified vs python3; positive/literal/nested indices unaffected. Found by a differential hunt |
| **PMAT-640** — runtime-negative list-index WRITE wraps (correctness) | `xs[i] = v`, `xs[i] += v` with i<0 | ✅ SHIPPED v0.1.339 — assign-side companion to PMAT-639. The subscript WRITE `xs[i as usize] = ...` panicked for runtime-negative `i` (the aug-assign read side already wrapped, but the store didn't). FIX (both backends' `Stmt::IndexAssign`, NO new IR): for a single non-negative-literal index, bind the wrapped index to a temp (`let __aidx = if __ai<0 { len+__ai } else { __ai }`) then store — which also ends `len`'s borrow before `index_mut`. Literal index keeps the bare path; nested `grid[i][j]=v` deferred. Verified vs python3 (incl. RHS-reads-list). Found by the differential hunt |
| **PMAT-641** — runtime-negative NESTED list-index write wraps (correctness) | `grid[-1][-1] = v` | ✅ SHIPPED v0.1.340 — completes negative-indexing (read 639, single write 640, nested write here). A nested write `grid[i][j]=v` didn't wrap a runtime-negative index at any level. FIX (both backends' `Stmt::IndexAssign`, NO new IR): generalize the single-index wrap to N levels — each level wraps via the progressively-indexed collection's own `len`, staged into temps first (subsumes the old `needs_temps` self-referential path → 3 codegen paths to 2). All-non-negative-literal write keeps the bare path. Verified vs python3. **Negative-indexing now complete.** Found by the differential hunt |
| **PMAT-642** — `enumerate(xs, start=-N)` negative start | `enumerate(xs, start=-1)` | ✅ SHIPPED v0.1.341 — **fixed reject**: a negative literal enumerate start (parsed `UnaryOp(USub, Int)`) was rejected ("non-literal start"); only a positive `Constant(Int)` was matched. FIX (frontend only, NO new IR): new `extract_int_literal` helper (signed literal incl. 0) used in the start parser; the codegen already `checked_add`s the start, so a negative one just offsets lower. Verified vs python3; positive/default unaffected. Found by a differential probe |
| **PMAT-643** — `str.isnumeric()` classification predicate | `s.isnumeric()` | ✅ SHIPPED v0.1.342 — **fixed reject**: `isnumeric` was an unrecognized str method. Added as a 0-arg Bool predicate (new `StrMethodOp::IsNumeric`, mirrors isdigit): both backends emit `(!(s).is_empty() && (s).chars().all(|__c| __c.is_numeric()))`. `char::is_numeric()` = Unicode Number categories, matching Python (broader than isdigit; `½`/`²` numeric-not-digit). Verified vs python3; isdigit unaffected. Found by a differential probe |
| **PMAT-644** — `str.rsplit(sep, maxsplit)` | `name.rsplit(".", 1)` | ✅ SHIPPED v0.1.343 — **fixed reject**: rsplit was unsupported. Splits from the right, capping at `maxsplit` splits. FIX (mirrors split=SplitN): new `StrMethodOp::RSplitN` (dedicated 2-arg branch); both backends emit `.rsplitn(maxsplit+1, sep).map(String).collect::<Vec<_>>().into_iter().rev().collect()` — `rsplitn` yields right-to-left, so the Vec is reversed to restore Python order. Negative maxsplit = no limit; bare `rsplit(sep)` maps to `Split`. Verified vs python3; split unaffected. Found by a differential probe |
| **PMAT-645** — starred unpacking `n0,…,*rest = xs` (star last) | `a, *rest = xs` | ✅ SHIPPED v0.1.344 — **fixed reject**: head/tail destructuring was unsupported. FIX (frontend desugar, NO new IR): a `lower_block_stmt` hook routes a tuple target whose last elt is `*name` to `lower_starred_unpack`, emitting `let n_i = xs[i]` (Index) per prefix name + `let star = xs[p:]` (Slice) for the rest. A plain-variable RHS is indexed/sliced directly (borrows, not moved); else a temp. Star-first/mid deferred. Verified vs python3; plain unpack unaffected. Found by a differential probe |
| **PMAT-646** — starred unpacking at any position | `*init, last`, `first, *mid, last` | ✅ SHIPPED v0.1.345 — extends PMAT-645 to a starred target ANYWHERE. FIX (frontend desugar, NO new IR): generalize `lower_starred_unpack` to prefix `let n_i = xs[i]` + star `let star = xs[p:len-s]` (Slice with a computed `len - s` hi) + suffix `let m_j = xs[len-(s-j)]` (computed Index). Star-last (s=0) reduces to PMAT-645. Verified vs python3. **Starred unpacking now complete (any single-star position).** Found by a differential probe |
| **PMAT-647** — starred-unpack binding `let mut` when mutated (correctness) | `a, *rest = xs; rest.append(...)` | ✅ SHIPPED v0.1.346 — **fixed transpile-invalid-rust** in PMAT-645/646: a mutated star binding emitted `let rest` (E0596). ROOT: the mutability pre-walk (`walk_counts`) skipped the Name inside a `Starred` tuple target, so its binding wasn't counted toward the mutable threshold. FIX (frontend `walk_counts`): count the `Name` inside `Starred`, mirroring the plain-Name arm. Verified vs python3; immutable-star stays `let`. **Found by HUNT-V2** (the corrected baked-in-binary hunt; surfaced this + 24 findings) |
| **PMAT-648** — `@dataclass(order=True)` derives `PartialOrd` (correctness) | `Point(1,2) < Point(1,3)` | ✅ SHIPPED v0.1.347 — **fixed transpile-invalid-rust (E0369)**: `order=True` generates ordering dunders in Python (instances compared field-by-field as a tuple), but xpile emitted `#[derive(Clone, Debug, PartialEq)]` with no `PartialOrd`, so `Inst < Inst` (and `<=`/`>`/`>=`) didn't compile. FIX (frontend + both backends, the dataclass decorator-arg path, +1 struct field): a new `class_has_order` helper (mirrors `class_is_frozen`, both refactored onto a shared `dataclass_kw_true(c, kw)` reader) detects `order=True`; `Item::Struct` gains an `order: bool` field; the codegens append `PartialOrd` to the derive list when set. `PartialOrd` gives lexicographic comparison by field order — matching Python's tuple comparison — sound for any comparable field incl. `f64` (full `Ord`, needed to sort instances, can't derive with a float field; deferred). Verified vs python3 (single/multi-field `<`, `>=`, float `<`); plain/frozen dataclasses unaffected. **Found by HUNT-V2** |
| **PMAT-649** — `str.split()` no-arg splits on C0 separators (correctness) | `"a\x1cb\x1dc".split()` | ✅ SHIPPED v0.1.348 — **fixed silent miscompile**: Python `str.split()` (no arg) splits on `str.isspace()` whitespace, which includes the C0 separators U+001C-1F (FS/GS/RS/US); xpile emitted Rust's `.split_whitespace()`, whose `char::is_whitespace` excludes them, so such strings split into fewer parts. FIX (both backends' `StrMethodOp::SplitWhitespace` arm, NO IR): emit `.split(\|__c\| __c.is_whitespace() \|\| matches!(__c, '\u{1c}'..='\u{1f}')).filter(\|s\| !s.is_empty())...` — the predicate reuses PMAT-600's C0-inclusive strip/isspace definition; the empty filter preserves no-arg split's leading/trailing/consecutive-collapse (no empties) that `split_whitespace` gave for free. Verified vs python3 (C0-only, mixed-collapse, plain ASCII, indexed); plain whitespace unaffected. **Found by HUNT-V2** |
