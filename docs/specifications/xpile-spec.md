...
- 🟡 Types beyond int/bool — **v0.2.0 Track 1.A foundation + contract substrate merged** (PMAT-449/450): `Type::Str` + `Expr::LitStr` end-to-end through depyler-frontend → Rust/Ruchy/Lean backends; **new contract `C-XLATE-PY-STR-TO-RUST-STRING`** at QUORUM (3 Lean theorems + 3 Kani harnesses + extrinsic + 1 Diamond theorem at depth-1). Substrate now: **13 contracts at QUORUM**, **172 Diamond theorems**, new contract ratchets through depth-2..13 in subsequent broadening sweeps. Concatenation, f-strings, `list[T]`, `dict[str,T]`, `float` are subsequent sub-tracks per [`sub/v0.2.0-depyler-merger.md`](sub/v0.2.0-depyler-merger.md).
