...
- 🟢 Types beyond int/bool — **v0.2.0 Track 1.A string lane SHIPPED** (PMAT-449/450/451/452): `Type::Str` + `Expr::LitStr` + `Expr::Concat` + f-string lowering all end-to-end through depyler-frontend → Rust/Ruchy/Lean backends; contract `C-XLATE-PY-STR-TO-RUST-STRING` at QUORUM, **depth-2** (utf8_bytes_preserved + concatenation_associativity). **The Track 1.A exit criterion `def greet(name: str) -> str: return f"Hello, {name}!"` transpiles cleanly to all three backends, rustc round-trip green.** Substrate now: **13 contracts at QUORUM**, **173 Diamond theorems**. `list[T]`, `dict[str,T]`, `float` are subsequent sub-tracks per [`sub/v0.2.0-depyler-merger.md`](sub/v0.2.0-depyler-merger.md).
