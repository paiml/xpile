...
| **PMAT-504** — closures / `lambda` | first-class fn values | 🔨 in progress — **first cut** `f = lambda y: <body>` → `Stmt::ClosureLet` (`let f = \|y: i64\| { … }`), callable `f(x)` via `Expr::Call`; single `i64` param, return type recorded in `ctx.closure_returns`; no `Type::Closure` (Rust infers); Lean refuses ✅ SHIPPED v0.1.89; multi-param / non-i64-param / closures-as-args follow |
