...
| **PMAT-506** — classes / dataclasses | Python `class`/`@dataclass` → Rust struct (+ impl) | 🔨 in progress — **first cut** field-only / `@dataclass` class → `Item::Struct` → `#[derive(Clone, Debug, PartialEq)] pub struct Name { pub <f>: <ty>, … }` (definition only; Lean refuses) ✅ SHIPPED v0.1.201 *(code shipped under the label `PMAT-505a` — a numbering slip, since PMAT-505 is `&str`; tracked here as PMAT-506)*. **Remaining:** value construction `Name(a, b)` + field access `obj.f` (need a `Type::Struct` variant), methods, field defaults, inheritance |
