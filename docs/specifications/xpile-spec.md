...
| **PMAT-484** | Lift `compile_targets.via` in the PTX contract YAML from comment-block to structured `role: general`/`specialist` records + an in-tree validator test (≥1 `role:general`, ≤1 general, no specialist-only route, tolerance present — honors §29 falsification posture #4). **Keep additive** alongside the flat list unless the pinned crates.io `aprender-contracts-cli` (pv) tolerates the structured shape — **confirm before landing** so `pv lint` stays green; the in-tree Rust test is the authoritative validator. (Cross-repo pv-engine enforcement stays the residual PMAT-A5.) | days | low | free runner (`pv lint` + in-tree test) |
...
| **PMAT-486** | `DiffExecEngine` trait + `Option<Arc<dyn DiffExecEngine>>` hook in `MultiEmitterBackend`; `NoopEngine` preserves today's `NotRun`. **Distinguish `NotRun{no-engine}` (benign) from a hard `DiffExecError` on an *installed* engine (must fail the GPU job)** — do not swallow errors into `NotRun`, or a broken GPU run could masquerade as "not run." | days | medium | free runner (NotRun preserved; existing PMAT-266/280 tests stay green) |
