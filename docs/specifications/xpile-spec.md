...
Implication: the Runtime-stratum `DiffExec` engine is no longer *hardware*-blocked — what remains is engineering (PMAT-26X+/26Z+: real emitters + the `DiffExec` comparison engine). This does **not** auto-schedule that work (it is larger GPU engineering outside the R-ladder), but the §4 "in-vacuum, cannot execute" caveat is now dischargeable on demand. The cuda-oxide watched bet (§30) inherits this: its hardware blocker is retired; its surviving gates are the unbuilt consumer here, the nightly+`rustc-dev` toolchain pin, and alpha maturity.
